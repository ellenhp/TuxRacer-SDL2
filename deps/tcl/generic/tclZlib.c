				 ...
				 * may get less! */
{
	 ...
	 */
		...
		while ((zshPtr->stream.avail_out > 0) && (e == Z_OK || e == Z_BUF_ERROR) && (listLen > 0))
		{
			/*
	 ...
	 */
		...
		while ((count > dataPos) &&
			   (Tcl_ListObjLength(NULL, zshPtr->outData, &listLen) == TCL_OK) && (listLen > 0))
		{
			/*
	...
	Tcl_Obj *const objv[])
{
	...
	case CMD_ADLER: /* adler32 str ?startvalue?
		...
		Tcl_SetObjResult(interp, Tcl_NewWideIntObj((Tcl_WideInt)(uLong)Tcl_ZlibAdler32(start, data, dlen)));
		return TCL_OK;
	case CMD_CRC: /* crc32 str ?startvalue?
		...
		data = Tcl_GetByteArrayFromObj(objv[2], &dlen);
		Tcl_SetObjResult(interp, Tcl_NewWideIntObj((Tcl_WideInt)(uLong)Tcl_ZlibCRC32(start, data, dlen)));
		return TCL_OK;
	 ...
	 */
		...
		if (objc == 4 && Tcl_GetIntFromObj(interp, objv[3], &level) == TCL_OK)
		{
			if (level < 0 || level > 9)
			{
				extraInfoStr = "\n    (in -level option)";
	...
	case CMD_INFLATE: /* inflate rawcomprdata ?bufferSize?
		...
		if (objc > 3)
		{
			...
			if (buffersize < MIN_NONSTREAM_BUFFER_SIZE || buffersize > MAX_BUFFER_SIZE)
			{
				goto badBuffer;
			}
		}
	...
	Tcl_Obj *const objv[])
{
	...
	case zs_checksum: /* $strm checksum */
		...
		Tcl_SetObjResult(interp, Tcl_NewWideIntObj((Tcl_WideInt)(uLong)Tcl_ZlibStreamChecksum(zstream)));
		return TCL_OK;
	...
	Tcl_Obj *const objv[])
{
	...
	else if (zshPtr->mode != TCL_ZLIB_STREAM_INFLATE || zshPtr->format != TCL_ZLIB_FORMAT_GZIP)
	{
		Tcl_SetObjResult(interp, Tcl_NewStringObj(
				 ...
				 * necessary. */
{
	...
	if (format == TCL_ZLIB_FORMAT_GZIP || format == TCL_ZLIB_FORMAT_AUTO)
	{
		...
		{
			cd->flags |= IN_HEADER;
			cd->inHeader.header.name = (Bytef *)&cd->inHeader.nativeFilenameBuf;
			cd->inHeader.header.name_max = MAXPATHLEN - 1;
			cd->inHeader.header.comment = (Bytef *)&cd->inHeader.nativeCommentBuf;
...
#define MAXBUF 1024
	 ...
	 */
		...
		if (((flush == Z_SYNC_FLUSH) && (e == Z_BUF_ERROR)) || (e == Z_STREAM_END) || (e == Z_OK && cd->inStream.avail_out == 0))
		{
			return TCL_OK;
		}

...
#include "deps/tcl/generic/tclInt.h"
#include "deps/tcl/generic/tclCompile.h"
#include "deps/tcl/generic/tclOOInt.h"
...
static const TalInstDesc TalInstructionTable[] = {
    ...
    {"push", ASSEM_PUSH, (INST_PUSH1 << 8 | INST_PUSH4), 0, 1},

    {"add", ASSEM_1BYTE, INST_ADD, 2, 1},
    {"append", ASSEM_LVT, (INST_APPEND_SCALAR1 << 8 | INST_APPEND_SCALAR4), 1, 1},
    {"appendArray", ASSEM_LVT, (INST_APPEND_ARRAY1 << 8 | INST_APPEND_ARRAY4), 2, 1},
    ...
    {"invokeStk", ASSEM_INVOKE, (INST_INVOKE_STK1 << 8 | INST_INVOKE_STK4), INT_MIN, 1},
    ...
    {"lappend", ASSEM_LVT, (INST_LAPPEND_SCALAR1 << 8 | INST_LAPPEND_SCALAR4),
     1, 1},
    {"lappendArray", ASSEM_LVT, (INST_LAPPEND_ARRAY1 << 8 | INST_LAPPEND_ARRAY4), 2, 1},
    ...
    {"load", ASSEM_LVT, (INST_LOAD_SCALAR1 << 8 | INST_LOAD_SCALAR4), 0, 1},
    {"loadArray", ASSEM_LVT, (INST_LOAD_ARRAY1 << 8 | INST_LOAD_ARRAY4), 1, 1},
    ...
    {"store", ASSEM_LVT, (INST_STORE_SCALAR1 << 8 | INST_STORE_SCALAR4), 1, 1},
    {"storeArray", ASSEM_LVT, (INST_STORE_ARRAY1 << 8 | INST_STORE_ARRAY4), 2, 1},
    ...
    Tcl_Obj *objPtr)    /* Source code to assemble */
{
    ...
    if (objPtr->typePtr == &assembleCodeType)
    {
        ...
        if (((Interp *)*codePtr->interpHandle == iPtr) && (codePtr->compileEpoch == iPtr->compileEpoch) && (codePtr->nsPtr == namespacePtr) && (codePtr->nsEpoch == namespacePtr->resolverEpoch) && (codePtr->localCachePtr == iPtr->varFramePtr->localCachePtr))
        {
            return codePtr;
        }
    ...
    AssemblyEnv *assemEnvPtr) /* State of the assembly */
{
    ...
    case ASSEM_CONCAT1:
        ...
        if (GetIntegerOperand(assemEnvPtr, &tokenPtr, &opnd) != TCL_OK || CheckOneByte(interp, opnd) != TCL_OK || CheckStrictlyPositive(interp, opnd) != TCL_OK)
        {
            goto cleanup;
        }
        BBEmitInstInt1(assemEnvPtr, tblIdx, opnd, opnd);
    ...
    case ASSEM_DICT_GET:
        ...
        if (GetIntegerOperand(assemEnvPtr, &tokenPtr, &opnd) != TCL_OK || CheckStrictlyPositive(interp, opnd) != TCL_OK)
        {
            goto cleanup;
        }
	 ...
	 * callback that calls TclCompileScript or TclCompileExpr. */
        ...
        {
            Tcl_WrongNumArgs(interp, 1, &instNameObj,
                             ((TalInstructionTable[tblIdx].tclInstCode == INST_EVAL_STK) ? "script" : "expression"));
            goto cleanup;
        }
        if (tokenPtr->type == TCL_TOKEN_SIMPLE_WORD)
        {
    ...
    case ASSEM_LIST:
        ...
        if (GetIntegerOperand(assemEnvPtr, &tokenPtr, &opnd) != TCL_OK || CheckNonNegative(interp, opnd) != TCL_OK)
        {
            goto cleanup;
        }
        BBEmitInstInt4(assemEnvPtr, tblIdx, opnd, opnd);
    ...
    case ASSEM_LVT1_SINT1:
        ...
        localVar = FindLocalVar(assemEnvPtr, &tokenPtr);
        if (localVar < 0 || CheckOneByte(interp, localVar) || GetIntegerOperand(assemEnvPtr, &tokenPtr, &opnd) != TCL_OK || CheckSignedOneByte(interp, opnd))
        {
            goto cleanup;
        }
        BBEmitInstInt1(assemEnvPtr, tblIdx, localVar, 0);
    ...
    case ASSEM_SINT1:
        ...
        if (GetIntegerOperand(assemEnvPtr, &tokenPtr, &opnd) != TCL_OK || CheckSignedOneByte(interp, opnd) != TCL_OK)
        {
            goto cleanup;
        }
        BBEmitInstInt1(assemEnvPtr, tblIdx, opnd, 0);
    ...
    *mustMove = 0;
    do
    {
		 ...
		 */
                ...
                {
                    offset = jumpTarget->startOffset - (bbPtr->jumpOffset + motion);
                    if (offset < -0x80 || offset > 0x7f)
                    {
                        opcode = TclGetUInt1AtPtr(envPtr->codeStart + bbPtr->jumpOffset);
                        ++opcode;
    ...
    int initialStackDepth)    /* Stack depth on entry to the block */
{
    ...
    if (blockPtr->enclosingCatch != 0 &&
        initialStackDepth + blockPtr->minStackDepth < (blockPtr->enclosingCatch->initialStackDepth + blockPtr->enclosingCatch->finalStackDepth))
    {
        if (assemEnvPtr->flags & TCL_EVAL_DIRECT)
        {
            Tcl_SetObjResult(interp, Tcl_NewStringObj(
	     ...
	     */
            ...
            while (i < bbPtr->successor1->startOffset)
            {
                ...
                if (opcode == INST_BEGIN_CATCH4)
                {
                    ...
                    if (catchIndex >= bbPtr->foreignExceptionBase && catchIndex < (bbPtr->foreignExceptionBase +

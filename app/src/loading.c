...
#ifdef ASYNC_LOADING
// En fait, la fonction loading_loop n'est pas une vrai fonction Loop, elle
// n'est executée qu'une fois, et comme ce n'est qu'une fois qu'on en sort que
// L'écran est redessiné, il faut la faire tourner au moins une fois sans rien
// loader pour que l'écran loading aparaisse, sinon il apparait quand tout est
// déjà chargé.
...
void loading_loop(scalar_t time_step) {
  ...
  if (loaded_course == NULL || loaded_course != g_game.race.course ||
...
#ifdef TR_DEBUG_MODE
      TRDebugLog(
          "(main thread) loading thread took %dms\n",

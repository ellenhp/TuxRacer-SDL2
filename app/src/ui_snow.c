...
static scalar_t frand() { return (scalar_t)rand() / RAND_MAX; }
...
void init_ui_snow(void) {
  ...
  num_particles = orig_num_particles =
      PARTICLE_DENSITY * getparam_x_resolution() * getparam_y_resolution();
...
void update_ui_snow(scalar_t time_step, bool_t windy) {
  ...
  for (i = 0; i < num_particles; i++) {
    ...
    dist_from_push =
        (pow((pt->x - push_position.x), 2) + pow((pt->y - push_position.y), 2));
    if (push_timestep > 0) {
      ...
      f.x *= 1.0 / (PUSH_DIST_DECAY * dist_from_push + 1) * size /
             PARTICLE_SIZE_RANGE;
      f.y *= 1.0 / (PUSH_DIST_DECAY * dist_from_push + 1) * size /
             PARTICLE_SIZE_RANGE;
    }

    /* Update velocity */
    v->x += (f.x + GRAVITY_FACTOR * grav_x + (windy ? WIND_FORCE : 0.0) -
             v->x * AIR_DRAG) *
            time_step;
    v->y += (f.y + GRAVITY_FACTOR * grav_y - v->y * AIR_DRAG) * time_step;
...
void init_arrays_for_particle(particle_t particle, GLfloat *vertices,
                              GLfloat *tex_coords) {
...
void draw_ui_snow(void) {
...

  {
    ...
    GLfloat *tex_coords =
        (GLfloat *)malloc(8 * num_particles * sizeof(GLfloat));
    GLushort *indices =
        (GLushort *)malloc(6 * num_particles * sizeof(GLushort));

    for (i = 0; i < num_particles; i++) {
      int firstvert = i * 4;
      int firstindex = i * 6;
      init_arrays_for_particle(particles[i], vertices + i * 8,
                               tex_coords + i * 8);
    ...
    // print_debug(DEBUG_OTHER, "indices %d %d %d %d %d %d %d %d %d %d %d %d",
    // indices[0], indices[1], indices[2], indices[3], indices[4], indices[5],
    // indices[6], indices[7], indices[8], indices[9], indices[10], indices[11]);
    // print_debug(DEBUG_OTHER, "vertices %f %f %f %f %f %f %f %f %f %f %f %f %f
    // %f %f %f", vertices[0], vertices[1], vertices[2], vertices[3],
    // vertices[4], vertices[5], vertices[6], vertices[7], vertices[8],
    // vertices[9], vertices[10], vertices[11], vertices[12], vertices[13],

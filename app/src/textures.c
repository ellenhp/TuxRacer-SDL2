...
bool_t load_texture(const char *texname, const char *filename, int repeatable) {
  ...
  print_debug(DEBUG_TEXTURE, "Loading texture %s from file: %s", texname,
              filename);
  ...
  if (get_hash_entry(texture_table, texname, (hash_entry_t *)&tex)) {
    print_debug(DEBUG_TEXTURE, "Found texture %s with id: %d", texname,
                tex->texture_id);
  ...
  if (texImage->w > max_texture_size || texImage->h > max_texture_size) {
  ...
  case 4:
    ...
    break;
  default:
    handle_error(1, "unsupported number of bytes per pixel in texture %s",
                 filename);
    break;
  }
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, format, texImage->w, texImage->h, 0, format,
...
bool_t bind_texture(const char *binding, const char *texname) {
  ...
  print_debug(DEBUG_TEXTURE, "Binding %s to texture name: %s", binding,
              texname);
...
void get_current_texture_dimensions(int *width, int *height) { abort(); }
...
static int load_texture_cb(ClientData cd, Tcl_Interp *ip, int argc,
  ...
  if (!load_texture(argv[1], argv[2], repeatable)) {
    Tcl_AppendResult(ip, argv[0], ": Could not load texture ", argv[2],
                     (char *)0);
    return TCL_ERROR;
  }
...
static int bind_texture_cb(ClientData cd, Tcl_Interp *ip, int argc,
  ...
  if (!bind_texture(argv[1], argv[2])) {
    Tcl_AppendResult(ip, argv[0], ": Could not bind texture ", argv[2],
                     (char *)0);
    return TCL_ERROR;
  }

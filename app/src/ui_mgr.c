...
/*!
  ...
  \date    Modified: 2000-09-16
*/
void ui_set_dirty() { needs_redraw = True; }
...
static void trigger_mouse_button_cbs(hash_table_t table,
                                     winsys_mouse_button_t button, int x,
                                     int y) {
  hash_search_t iter;
  ui_callback_data_t *cb_data;
  ...
  while (next_hash_entry(iter, NULL, (hash_entry_t *)&cb_data)) {
    ((mouse_button_event_cb_t)cb_data->cb)(cb_data->widget,
                                           (winsys_mouse_button_t)button, x, y);
...
/*!
  ...
  \date    Modified: 2000-09-16
*/
void ui_event_mouse_func(int button, int state, int finger_index, int x,
                         int y) {
  if (is_mode_change_pending()) {
...
/*!
  ...
  \date    Modified: 2000-09-29
*/
point2d_t ui_get_mouse_position(void) { return cursor_pos; }

...
#ifdef TARGET_OS_IPHONE
  if (g_game.practicing && !g_game.race_aborted && g_game.race.name != NULL &&
      did_player_beat_best_results() && g_game.rankings_displayed == False) {
    // Notify that a new best result is for the moment unsaved
    // dirtyScores();
  }

  if (!did_player_beat_best_results() &&
      !plyrWantsToDisplayRankingsAfterRace()) {
    g_game.rankings_displayed = True;
  }

  if (!g_game.race_aborted && g_game.practicing &&
      plyrWantsToSaveOrDisplayRankingsAfterRace() &&
...
void game_over_set_next_mode(int mode) { next_mode = mode; }
...
void draw_game_over_text(void) {
  ...
  if (!g_game.race_aborted && g_game.practicing &&
      g_game.needs_save_or_display_rankings) {
...
void displaySavedAndRankings(const char *msg, const char *friends,
                             const char *country, const char *world,
                             scalar_t friendsPercentage,
                             scalar_t countryPercentage,
                             scalar_t worldPercentage) {
  free(friendsRanking);
...
void saveAndDisplayRankings() {
  ...
  if (g_game.practicing && !g_game.race_aborted && g_game.race.name != NULL &&
    ...
    // if the player choosed in his prefs not to save score online after ending
    // a race but just to display rankings, the function below will detect this
    // case and redirect to the function displayRankingsAfterRace
  }
  // else display world rankings for this score
  else if (g_game.practicing && !g_game.race_aborted &&
           g_game.race.name != NULL && !did_player_beat_best_results()) {
    int minutes;
    int seconds;
    int hundredths;
    player_data_t *plyr = get_player_data(local_player());
...
void game_over_init(void) {
  ...
  if ((!g_game.practicing && !aborted) ||
      (!g_game.practicing && aborted && !game_abort_is_for_tutorial())) {
...
void game_over_register() {
  ...
  status |=
      add_keymap_entry(GAME_OVER, DEFAULT_CALLBACK, NULL, NULL, game_over_cb);

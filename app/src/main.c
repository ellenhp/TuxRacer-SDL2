...
int tux_eval(Tcl_Interp *interp, char *filename) {
  ...
  if (Tcl_Eval(interp, tcl_script_buf) == TCL_ERROR) {
    handle_error(1, "error evalating %s: %s", filename,
...
int tcl_tux_eval(ClientData clientData, Tcl_Interp *interp, int argc,
                 char *argv[]) {
  if (argc != 2) {
...
void read_game_init_script() {
  ...
  Tcl_CreateCommand(g_game.tcl_interp, "tux_eval", (Tcl_CmdProc *)tcl_tux_eval,
                    0, NULL);

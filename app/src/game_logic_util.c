...
bool_t is_current_race_last_race_in_cup(void) {
  ...
  check_assertion(!g_game.practicing,
...
bool_t did_player_beat_best_results(void) {
  ...
  if (!get_saved_race_results(plyr->name, g_game.current_event,
                              g_game.current_cup, g_game.race.name,
                              g_game.difficulty, &time, &herring, &score)) {
#ifdef TARGET_OS_IPHONE
    /* No previous result, so we didn't "beat" anything : OK BUT WE STILL WANT
     * TO SAVE THE SCORE ONLINE !!!!! */
    return True;
#else
...
int calculate_player_score(player_data_t *plyr) {
  ...
  {
    // Quand tux ne vole pas,on fixe flying_time à plyr->control.fly_total_time
    // (surtout utile au début de la course, car c'est une variable statique,
    // ainsi ca la remet à zéro)
    if (!plyr->control.is_flying) {
      ...
      if (g_game.time - plyr->control.fly_start_time > FLYING_TIME_LIMIT) {
        flying_time = plyr->control.fly_total_time + g_game.time -
                      plyr->control.fly_start_time;
      }
    }
  ...
  /* score calculation */
  score =
      max(0, (int)(time_bonus + herring_bonus + flying_bonus + tricks_bonus));

...
static void update_race_results(void) {
  ...
  if (!get_saved_race_results(plyr->name, event, cup, race_name,
  ...
  if (update_score) {
    bool_t result;
    result = set_saved_race_results(plyr->name, event, cup, race_name,
                                    g_game.difficulty, g_game.time,
...
void update_for_won_race(void) {
  ...
  if (last_completed_race == NULL ||
      compare_race_positions(cup_data, last_completed_race, cur_elem) > 0) {
    last_completed_race = cur_elem;

    if (cur_elem == get_list_tail(race_list)) {
      cup_complete = True;

      if (!set_last_completed_cup(plyr->name, g_game.current_event,
                                  g_game.difficulty, g_game.current_cup)) {
...
void play_cb(int button, int mouse_x, int mouse_y, widget_bounding_box_t bb,
             input_type_t input_type, widget_t *widget) {
  start_race();
}

void back_cb(int button, int mouse_x, int mouse_y, widget_bounding_box_t bb,
             input_type_t input_type, widget_t *widget) {
  back();
}

void prev_cb(int button, int mouse_x, int mouse_y, widget_bounding_box_t bb,
             input_type_t input_type, widget_t *widget) {
...
void next_cb(int button, int mouse_x, int mouse_y, widget_bounding_box_t bb,
             input_type_t input_type, widget_t *widget) {
...
static void race_select_init(void) {
  ...
  if (g_game.prev_mode != GAME_OVER) {
    ...
    if (g_game.practicing) {
      ...
      if (is_cup_complete(event_data, get_event_cup_by_name(
                                          event_data, g_game.current_cup))) {
        cup_complete = True;
   ...
   */
  desc_ta =
      textarea_create(make_point2d(0.1 * getparam_x_resolution(),
                                   0.2 * getparam_y_resolution()),
                      0.4 * getparam_x_resolution(),
                      0.23 * getparam_y_resolution(), "race_description", "");
  ...
  gui_add_widget(prev_course_btn = create_button(LEFT_ARROW, prev_cb),
                 &button_coord);

  button_coord.x = 0.9;
  button_coord.x_just = RIGHT_JUST;
  gui_add_widget(next_course_btn = create_button(RIGHT_ARROW, next_cb),
                 &button_coord);
  ...
  button_coord.y = 0.09;
  button_coord.x_just = CENTER_JUST;
  gui_add_widget(back_button = create_button(get_back_text(), back_cb),
                 &button_coord);

  button_coord.x = 0.70;
  gui_add_widget(play_button = create_button(get_race_text(), play_cb),
                 &button_coord);
...
void race_select_register() {
  ...
  status |= add_keymap_entry(RACE_SELECT, DEFAULT_CALLBACK, NULL, NULL,
                             race_select_key_cb);

  check_assertion(status == 0, "out of keymap entries");

  register_loop_funcs(RACE_SELECT, race_select_init, race_select_loop,
                      race_select_term);
}

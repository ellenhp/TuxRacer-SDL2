...
void init_programs() {
  ...
  init_shader_program(&generic_program, "shaders/generic.vert",
                      "shaders/generic.frag");
  init_shader_program(&hud_program, "shaders/hud.vert", "shaders/hud.frag");
  init_shader_program(&terrain_program_high, "shaders/terrain.vert",
                      "shaders/terrain.frag");
  init_shader_program(&terrain_program_low, "shaders/terrainlow.vert",
                      "shaders/terrainlow.frag");
  init_shader_program(&tree_program, "shaders/trees.vert",
                      "shaders/trees.frag");
...
static bool_t use_program(GLuint program) {
  if (programs_initialized && active_program != program) {
    ...
    set_MVP();
    set_light_uniforms();
    glUniform1f(shader_get_uniform_location(SHADER_CLIP_NAME),
                getparam_forward_clip_distance() / 3);
    return True;
  }
  return False;
}
...
void use_generic_program() {
  if (use_program(generic_program)) {
    glUniform1f(glGetUniformLocation(active_program, "course_angle"),
                get_course_angle() / 180 * 3.14159);
    set_light_uniforms();
  }
}

void use_hud_program() { use_program(hud_program); }
...
void use_tux_program() { use_program(tux_program); }

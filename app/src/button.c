...
static bool_t in_bbox(button_t *button, int x, int y) {
  return (bool_t)((x >= button->pos.x) && (x <= button->pos.x + button->w) &&
                  (y >= button->pos.y) && (y <= button->pos.y + button->h));
}
...
static void button_mouse_down_cb(void *widget,
                                 winsys_mouse_button_t which_button, int x,
                                 int y) {
...
/*!
 ...
 \date    Modified: 2000-09-17
 */
static void button_mouse_up_cb(void *widget, winsys_mouse_button_t which_button,
                               int x, int y) {
  button_t *button = (button_t *)widget;
...
void button_draw(button_t *button) {
  ...
  if (!button->enabled) {
  ...
  } else if (button->clicked && button->focused) {
    ...
    /*Si le bouton est du texte, on ajuste sa largeur à la largeur du texte,
     * pour pouvoir le placer plus precisement*/
  ...
  if (tex != NULL) {
    if (!get_texture_binding(tex->binding, &texobj)) {
      print_warning(IMPORTANT_WARNING,
                    "Couldnt get texture object for binding %s", tex->binding);
      texobj = 0;
    }
    {
      GLfloat texcoords[] = {tex->ll.x, tex->ll.y, tex->ll.x, tex->ur.y,
                             tex->ur.x, tex->ur.y, tex->ur.x, tex->ll.y};
...
/*!
 ...
 \date    Modified: 2000-09-17
 */
static void button_draw_cb(void *widget) { button_draw((button_t *)widget); }
...
button_t *button_create(point2d_t pos, scalar_t w, scalar_t h,
  ...
  /*Si le bouton est du texte, on ajuste sa largeur à la largeur du texte, pour
   * pouvoir le placer plus precisement*/
  if (button->font_binding) {
    font_binding = button->font_binding;
    if (button->label != NULL) {
      ...
      if (!get_font_binding(font_binding, &font)) {
        print_warning(IMPORTANT_WARNING,
                      "Couldn't get font object for binding %s", font_binding);
        font = NULL;
      } else {
...
/*!
 ...
 \date Modified:    2000-09-17
 */
void button_set_image(button_t *button, char *texture_binding, point2d_t p0,
                      point2d_t p1, colour_t colour) {
  check_assertion(button != NULL, "button is NULL");

...
typedef enum { PARAM_STRING, PARAM_CHAR, PARAM_INT, PARAM_BOOL } param_type;
...
#define FN_PARAM(name, typename, type)                                         \
  ...
  void setparam_##name(type val) { set_param_##typename(&(Params.name), val); }
...
void init_game_configuration() {
...
#ifdef TARGET_OS_IPHONE
  INIT_PARAM_STRING(data_dir, strdup(getRessourcePath()),
  ...
  INIT_PARAM_INT(x_resolution, 320,
  ...
  INIT_PARAM_BOOL(fullscreen, True,
  ...
  INIT_PARAM_STRING(turn_right_key, "l right",
  ...
  INIT_PARAM_STRING(reset_key, "backspace",
  ...
  INIT_PARAM_INT(joystick_jump_button, 3,
                 "# Joystick button for jumping (numbering starts at 0)");

  INIT_PARAM_INT(joystick_trick_button, 1,
  ...
  INIT_PARAM_INT(joystick_x_axis, 0,
  ...
  INIT_PARAM_INT(fov, 70, "# [EXPERT] Sets the camera field-of-view");
...
int getparam_x_resolution(void) { return Android_ScreenWidth; }

void setparam_x_resolution(int width) {}

int getparam_y_resolution(void) { return Android_ScreenHeight; }
...
void write_config_file() {
  ...
  if (config_stream == NULL) {
    print_warning(CRITICAL_WARNING, "couldn't open %s for writing: %s",
                  config_file, strerror(errno));
    return;
  }

  sprintf(buf, "# Tux Racer " VERSION " configuration file\n"
               "#\n");
  ...
  for (i = 0; i < sizeof(Params) / sizeof(struct param); i++) {
    parm = (struct param *)&Params + i;
#ifdef TARGET_OS_IPHONE
    // FIXME : dans la version simulateur, on ne veut as qu'il enregistre le
    // data_dir car il change à chaque fois et sinon c'est la merde
    if (!strcmp(parm->name, "data_dir"))
      continue;

    // Don't save the resolution
    if (!strcmp(parm->name, "x_resolution"))
      continue;
    if (!strcmp(parm->name, "y_resolution"))
      continue;
#endif
 ...
 */
static int get_param_cb(ClientData cd, Tcl_Interp *ip, int argc,
                        const char *argv[]) {
  int i;
  int num_params;
  struct param *parm;
  ...
  if (parm == NULL || i == num_params) {
    Tcl_AppendResult(ip, argv[0], ": invalid parameter `", argv[1], "'",
                     (char *)0);
    return TCL_ERROR;
  }
 ...
 */
static int set_param_cb(ClientData cd, Tcl_Interp *ip, int argc,
                        const char *argv[]) {
  int i;
  int tmp_int;
  int num_params;
  ...
  case PARAM_CHAR:
    if (strlen(argv[2]) > 1) {
      Tcl_AppendResult(ip, "\n", argv[0], ": value for `", argv[1],
                       "' must be a single character", (char *)0);
      return TCL_ERROR;
    }

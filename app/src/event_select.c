...
static void set_widget_positions_and_draw_decorations() {
  ...
  button_set_position(
      back_btn,
      make_point2d(x_org + 40 - button_get_width(back_btn) / 2.0, 12));

  button_set_position(
      continue_btn,
      make_point2d(x_org + 232 - button_get_width(continue_btn) / 2.0, 12));

  button_set_position(
      howToPlay_btn,
      make_point2d(x_org + 160 - button_get_width(continue_btn) / 2.0, 100));
  // Out of the field of view
  // in Tux Rider World Challenge, there is no more events and cup, just
  // training I'm adapting from The Tux Racer system by selecting the first and
  // only event I hide its list box out of the field of view and each cup for
  // this event is a training

  listbox_set_position(event_listbox, make_point2d(x_org + 500, y_org + 500));

  listbox_set_position(cup_listbox, make_point2d(x_org + 50, y_org + 193));
  ...
  button_set_position(
      back_btn,
      make_point2d(x_org + 131 - button_get_width(back_btn) / 2.0, 42));

  button_set_position(
      continue_btn,
      make_point2d(x_org + 329 - button_get_width(continue_btn) / 2.0, 42));
  ...
  if ( !get_texture_binding(get_cup_icon_texture_binding((cup_data_t*)
get_list_elem_data( cur_cup ) ), &texobj ) )
  {
      texobj = 0;
  }
...
static void event_select_init(void) {
  ...
  /* How to play button */
  howToPlay_btn =
      button_create(dummy_pos, 150, 40, "button_label", "How to play ?");
  ...
  /* event listbox */
  event_listbox = listbox_create(dummy_pos, BOX_WIDTH - 52, 44, "listbox_item",
                                 event_list, event_list_elem_to_string_func);

  listbox_set_current_item(event_listbox, cur_event);

  listbox_set_item_change_event_cb(event_listbox, event_listbox_item_change_cb,
                                   NULL);
  ...
  cup_listbox = listbox_create(dummy_pos, BOX_WIDTH - 52, 44, "listbox_item",
                               cup_list, cup_list_elem_to_string_func);

  listbox_set_current_item(cup_listbox, cur_cup);

  listbox_set_item_change_event_cb(cup_listbox, cup_listbox_item_change_cb,
                                   NULL);
...
void event_select_register() {
  ...
  status |= add_keymap_entry(EVENT_SELECT, DEFAULT_CALLBACK, NULL, NULL,
                             event_select_key_cb);

  check_assertion(status == 0, "out of keymap entries");

  register_loop_funcs(EVENT_SELECT, event_select_init, event_select_loop,
                      event_select_term);
}

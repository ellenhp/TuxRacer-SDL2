  ...
  if (getparam_no_audio() == False) {
    ...
    if (Mix_OpenAudio(hz, format, channels, buffer) < 0) {
      print_warning(1,
                    ...
                    "  Reason: %s\n",
                    hz, getparam_audio_format_mode() == 0 ? 8 : 16,
                    SDL_GetError());
    } else {
      print_debug(DEBUG_SOUND, "Opened audio device at %d Hz %d-bit audio", hz,
...
bool_t is_music_playing() {
  ...
  check_assertion(
      (current_music_name_ == NULL && current_music_data_ == NULL) ||
          (current_music_name_ != NULL && current_music_data_ != NULL),
...
/*!
  ...
  \date    Modified: 2000-08-14
*/
static int bind_sounds_cb(ClientData cd, Tcl_Interp *ip, int argc,
                          char *argv[]) {
  check_assertion(initialized_, "audio module not initialized");
...
/*!
  ...
  \date    Modified: 2000-08-14
*/
static int bind_music_cb(ClientData cd, Tcl_Interp *ip, int argc,
                         char *argv[]) {
  int loops;
...
void init_audio() { DPRINTF("init_audio\n"); }
...
bool_t mustLoopFromContext(char *context) {
  if (!strcmp(context, "game_over"))
    return False;
  else
    return True;
}

// Certains sons sont trop courts pour être joués avec audioQueueServices (j'en
// suis pas sur mais en tout cas ca plante) Dans ce cas je les joue avec
// AudioServicesPlaySystemSound (SS= systemSound)
bool_t isSSFromContext(char *context) {
  if (!strcmp(context, "item_collect"))
    return True;
  else if (!strcmp(context, "hit_tree"))
    return True;
  else
    return False;
}

// Cette fonction est utile car certains contextes portent des noms différents
// et servent à jouer la même musique
char *uniqueContextFromContext(char *context) {
  if (!strcmp(context, "splash_screen"))
    return "start_screen";
  // else if (!strcmp(context,"loading")) return "start_screen";
  else if (!strcmp(context, "intro"))
    return "racing";
  else if (!strcmp(context, "paused"))
    return "racing";
  else
    return context;
}
...
bool_t play_music(char *music_context) {
  ...
  playIphoneMusic(uniqueContextFromContext(music_context),
                  mustLoopFromContext(music_context));
  return False;
}
...
void shutdown_audio() { DPRINTF("shutdown_audio\n"); }

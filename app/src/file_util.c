...
list_t get_dir_file_list(char *dirname) {
  ...
  do {
    cur_elem =
        insert_list_elem(dirlist, cur_elem, string_copy(finddata.cFileName));
  ...
  while ((cur_entry = readdir(dir_stream)) != NULL) {
    cur_elem =
        insert_list_elem(dirlist, cur_elem, string_copy(cur_entry->d_name));
  }
...
void free_dir_file_list(list_t dirlist) {
  ...
  for (cur_elem = get_list_head(dirlist); cur_elem != NULL;

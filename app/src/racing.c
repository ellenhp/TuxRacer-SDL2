...
/* factor limit to be considered as a left or a right turn. useful only for
 * tricks */
...
bool_t get_trick_modifier() { return trick_modifier; }

/* ce init ne ser que pendant le tutorial, quand on souhaite faire revenir tux
 * en arriere */
...
void racing_mouse_func(int button, int state, int finger_index, int absolute_x,
                       int absolute_y) {
...
void racing_init(void) {
  ...
  if (winsys_is_controller_active() == False) {
  ...
  winsys_add_js_button_binding(SDL_CONTROLLER_BUTTON_Y,
                               getparam_trick_modifier_key()[0]);
  winsys_add_js_button_binding(SDL_CONTROLLER_BUTTON_B,
                               getparam_pause_key()[0]);
  ...
  if (g_game.practicing) {
    ...
    /* en mode training il faut qu'on voie tux pour voir quand il paddle, quand
     * il fait des tricks, etc... */
...
void calc_jump_amt(scalar_t time_step) {
  ...
  if (plyr->control.jump_charging) {
    ...
    plyr->control.jump_amt *=
        (1.0 -
         (g_game.time - plyr->control.jump_start_time) / JUMP_FORCE_DURATION);
  } else {
...
void racing_loop(scalar_t time_step) {
  ...
  if (touch_control) {
    braking =
        (bool_t)((SDL_GetTicks() - last_brake_tap) < TAP_EFFECT_LENGTH_TICKS);
  }
  ...
  if (airborne) {
    ...
    if (touch_control) {
      trick_modifier =
          (bool_t)((SDL_GetTicks() - last_trick_tap) < TAP_EFFECT_LENGTH_TICKS);
    }
  ...
  if ((left_turn || joy_left_turn) ^ (right_turn || joy_right_turn)) {
    ...
    plyr->control.turn_animation +=
        (turning_left ? -1 : 1) * 0.15 * time_step / 0.05;
    plyr->control.turn_animation =
  ...
  if (touch_control) {
    paddling =
        (bool_t)((SDL_GetTicks() - last_paddle_tap) < TAP_EFFECT_LENGTH_TICKS);
  }
  ...
  if (new_terrain & (1 << NumTerrains)) {
    ...
    if (last_terrain & (1 << NumTerrains)) {
      ...
      if (plyr->control.fly_end_time - plyr->control.fly_start_time >
          FLYING_TIME_LIMIT) {
        plyr->control.fly_total_time +=
            plyr->control.fly_end_time - plyr->control.fly_start_time;
      }
#endif
  ...
  if (roll && plyr->control.barrel_roll_factor == 0) {
    if (flip) {
      if (plyr->control.barrel_roll_left)
        add_new_bonus("Hyper heavy Jump",
                      get_score_for_trick(HYPER_HEAVY_JUMP));
      else
        add_new_bonus("Ray star hybrid Jump",
                      get_score_for_trick(RAY_STAR_HYBRID_JUMP));
    } else {
      if (plyr->control.barrel_roll_left)
  ...
  if (flip && plyr->control.flip_factor == 0) {
    if (roll) {
      if (plyr->control.back_flip)
        add_new_bonus("Saturn ice Fever",
                      get_score_for_trick(SATURN_ICE_FEVER));
      else
        add_new_bonus("Wild pinguin Show",
                      get_score_for_trick(WILD_PINGUIN_SHOW));
    } else {
      if (plyr->control.back_flip)
...
void racing_register() {
  ...
  status |=
      add_keymap_entry(RACING, DEFAULT_CALLBACK, NULL, NULL, racing_default_cb);

  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "escape",
                             getparam_quit_key, quit_racing_cb);
  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "j",
                             getparam_turn_left_key, turn_left_cb);
  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "l",
                             getparam_turn_right_key, turn_right_cb);
  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "d",
                             getparam_trick_modifier_key, trick_modifier_cb);
  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "space",
                             getparam_brake_key, brake_cb);
  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "k", getparam_paddle_key,
                             paddle_cb);

  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "1",
                             getparam_behind_view_key, behind_view_cb);
  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "2",
                             getparam_follow_view_key, follow_view_cb);
  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "3",
                             getparam_above_view_key, above_view_cb);

  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "=",
                             getparam_screenshot_key, screenshot_cb);
  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "p", getparam_pause_key,
                             pause_cb);
  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "r", getparam_reset_key,
                             reset_cb);
  status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "i", getparam_jump_key,
                             jump_cb);

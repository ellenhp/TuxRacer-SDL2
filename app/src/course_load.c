...
void get_gl_arrays(GLubyte **vnc_arr) { *vnc_arr = vnc_array; }
...
static void free_polygon_list(int num_polygons, polygon_t *polygon_list) {
  int i;
...
static void reset_course() {
  ...
  for (i = 0; i < num_tree_types; i++) {
    ...
    free_polygon_list(tree_types[i].poly.num_polygons,
                      tree_types[i].poly.polygons);
...
#undef floatval
#define byteval(i)                                                             \
  (*(GLubyte *)(vnc_array + idx + 10 * sizeof(GLfloat) + i * sizeof(GLubyte)))
...
void preload_course(char *course) { reset_course(); }
...
void load_course_core(char *course) {
  ...
  if (Tcl_Eval(g_game.tcl_interp, pre_script) == TCL_ERROR) {
    handle_error(1, "Error evaluating %s pre_script: %s", buf,
                 Tcl_GetStringResult(g_game.tcl_interp));
  }
  if (Tcl_Eval(g_game.tcl_interp, script_buf) == TCL_ERROR) {
    handle_error(1, "Error evaluating %s: %s", buf,
                 Tcl_GetStringResult(g_game.tcl_interp));
  }
  ...
  init_course_vbo(elevation, terrain, nx, ny, course_width / (nx - 1.),
                  -course_length / (ny - 1));
...
void load_course(char *course) {
  preload_course(course);
  load_course_core(course);
  postload_course(course);
}
...
static int course_dim_cb(ClientData cd, Tcl_Interp *ip, int argc,
                         const char *argv[]) {
  double width, length;
...
static int angle_cb(ClientData cd, Tcl_Interp *ip, int argc,
                    const char *argv[]) {
  double angle;
...
static int elev_cb(ClientData cd, Tcl_Interp *ip, int argc,
                   const char *argv[]) {
  ...
  for (y = 0; y < ny; y++) {
    for (x = 0; x < nx; x++) {
      pixel_value = ((unsigned char *)
                         elev_img->pixels)[x * elev_img->format->BytesPerPixel +
                                           y * elev_img->pitch];
      ELEV(nx - 1 - x, ny - 1 - y) =
          ((pixel_value - base_height_value) / 255.0) * elev_scale -
...
static int terrain_cb(ClientData cd, Tcl_Interp *ip, int argc,
                      const char *argv[]) {
  ...
  for (y = 0; y < ny; y++) {
    for (x = 0; x < nx; x++) {
      ...
      terrain[idx] = intensity_to_terrain(
          ((unsigned char *)
               terrain_img->pixels)[x * terrain_img->format->BytesPerPixel +
                                    y * terrain_img->pitch]);
    }
  }
...
static int bgnd_img_cb(ClientData cd, Tcl_Interp *ip, int argc,
                       const char *argv[]) {
...
// deprecated
*/
static int tree_tex_cb(ClientData cd, Tcl_Interp *ip, int argc,
                       const char *argv[]) {
...
static int ice_tex_cb(ClientData cd, Tcl_Interp *ip, int argc,
                      const char *argv[]) {
...
static int rock_tex_cb(ClientData cd, Tcl_Interp *ip, int argc,
                       const char *argv[]) {
...
static int snow_tex_cb(ClientData cd, Tcl_Interp *ip, int argc,
                       const char *argv[]) {
...
static int start_pt_cb(ClientData cd, Tcl_Interp *ip, int argc,
                       const char *argv[]) {
  double xcd, ycd;
...
static int elev_scale_cb(ClientData cd, Tcl_Interp *ip, int argc,
                         const char *argv[]) {
...
*/
static int trees_cb(ClientData cd, Tcl_Interp *ip, int argc,
                    const char *argv[]) {
  SDL_Surface *treeImg;
  int sx, sy, sz;
  int x, y;
  ...
  for (y = 0; y < sy; y++) {
    for (x = 0; x < sx; x++) {
      best_tree_dist =
          is_tree(((unsigned char *)treeImg->pixels) + (x + y * sx) * sz + pad,
                  &which_tree);
      best_item_dist =
          is_item(((unsigned char *)treeImg->pixels) + (x + y * sx) * sz + pad,
                  &which_item);
      ...
      if (best_tree_dist < best_item_dist && which_tree != NULL) {
        ...
        which_tree->insert_pos = insert_list_elem(
            which_tree->pos, which_tree->insert_pos, (list_elem_data_t)pos);
        ...
        pos->x = (sx - x) / (scalar_t)(sx - 1.) * course_width;
        pos->y = -(sy - y) / (scalar_t)(sy - 1.) * course_length;
        which_item->insert_pos = insert_list_elem(
            which_item->pos, which_item->insert_pos, (list_elem_data_t)pos);
      }
    }
  ...
  */
  ...
  for (i = 0; i < num_item_types; i++) {
    ...
    while (elem != NULL) {
      ...
      item_locs[num_items].ray.pt.y =
          find_y_coord(pos->x, pos->y) + item_types[i].above_ground;
      free(pos);
...
// deprecated
*/
static int tree_size_cb(ClientData cd, Tcl_Interp *ip, int argc,
                        const char *argv[]) {
  double diam, height;
 ...
 */
static int get_polyhedron_vertices(Tcl_Interp *ip, const char *arg,
                                   int *num_vertices, point_t **vertex_list) {
  int i;
  const char **indices = 0;
  int rtn;
  scalar_t pt[3];
...
get_single_polygon(Tcl_Interp *ip, const char *arg, polygon_t *polygon) {
  int i;
  const char **indices = 0;
  int rtn;
  int num_vertices;
 ...
 */
static int get_polyhedron_polygon(Tcl_Interp *ip, const char *arg,
...
// deprecated
*/
static int tree_poly_cb(ClientData cd, Tcl_Interp *ip, int argc,
                        const char *argv[]) {
  int num_vertices;
  point_t *vertices;
  int num_polygons;
  ...
  if (get_polyhedron_vertices(ip, argv[1], &num_vertices, &vertices) == TCL_OK)
    if (get_polyhedron_polygon(ip, argv[2], &num_polygons, &polygons) ==
        TCL_OK) {
...
static int friction_cb(ClientData cd, Tcl_Interp *ip, int argc,
                       const char *argv[]) {
  double fric[3];
  scalar_t fric_s[3];
  int i;
...
static int course_author_cb(ClientData cd, Tcl_Interp *ip, int argc,
                            const char *argv[]) {
  if (argc != 2) {
    Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n",
...
static int course_name_cb(ClientData cd, Tcl_Interp *ip, int argc,
                          const char *argv[]) {
  if (argc != 2) {
    Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n",
...
static int calculation_mode_cb(ClientData cd, Tcl_Interp *ip, int argc,
                               const char *argv[]) {
  if (argc != 2) {
    Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n",
                     "Usage: ", argv[0], " <calculation mode>", (char *)0);
    return TCL_ERROR;
  }
...
static int base_height_value_cb(ClientData cd, Tcl_Interp *ip, int argc,
                                const char *argv[]) {
  int value;
...
static int tree_props_cb(ClientData cd, Tcl_Interp *ip, int argc,
                         const char *argv[]) {
  int i;
  int num_vertices;
  point_t *vertices;
  ...
  for (i = 1; (i < argc - 1) && !error; i += 2) {
    if (strcmp("-name", argv[i]) == 0) {
      ...
      if (Tcl_GetDouble(ip, argv[i + 1], &tree_types[num_tree_types].diam) !=
          TCL_OK) {
        Tcl_AppendResult(ip, argv[0], ": invalid diameter", (char *)0);
        error = 1;
      }
      ...
      if (Tcl_GetDouble(ip, argv[i + 1], &tree_types[num_tree_types].height) !=
          TCL_OK) {
        Tcl_AppendResult(ip, argv[0], ": invalid height", (char *)0);
        error = 1;
      }
      ...
      if (rtn != TCL_OK) {
        ...
        Tcl_AppendResult(ip, argv[0],
                         ": must specify three colours"
                         " to link with tree type",
                         (char *)0);
        error = 1;
      }
      Tcl_Free((char *)indices);
      ...
      if (rtn != TCL_OK || num_col != 2) {
        Tcl_AppendResult(ip,
                         "two sublists of vertices and polygons"
                         " must be specified\n",
                         (char *)0);
        Tcl_Free((char *)indices);
        error = 1;
      ...
      if (Tcl_GetDouble(ip, argv[i + 1], &tree_types[num_tree_types].vary) !=
          TCL_OK) {
  ...
  if (tree_types[num_tree_types].name == 0 ||
    ...
    Tcl_AppendResult(
        ip, argv[0], ": some mandatory elements not filled",
    ...
    free_polygon_list(tree_types[num_tree_types].poly.num_polygons,
                      tree_types[num_tree_types].poly.polygons);
    return TCL_ERROR;
...
static int item_spec_cb(ClientData cd, Tcl_Interp *ip, int argc,
                        const char *argv[]) {
  int rtn, num_col;
  ...
  while (*argv != NULL) {
    if (strcmp("-name", *argv) == 0) {
      ...
      if (Tcl_GetDouble(ip, *argv, &item_types[num_item_types].height) !=
          TCL_OK) {
      ...
      if (Tcl_GetDouble(ip, *argv, &item_types[num_item_types].diam) !=
          TCL_OK) {
      ...
      if (Tcl_GetInt(ip, *argv, &item_types[num_item_types].atlas_index) !=
          TCL_OK) {
      ...
      if (Tcl_GetDouble(ip, *argv, &item_types[num_item_types].above_ground) !=
          TCL_OK) {
      ...
      if (get_tcl_tuple(ip, *argv,
                        (scalar_t *)&(item_types[num_item_types].normal),
                        3) != TCL_OK) {
...
void set_light_uniforms() {
  ...
  glUniform3fv(shader_get_uniform_location(SHADER_LIGHT_POSITION_NAME), 1,
               course_lights[0].position);
  glUniform4fv(shader_get_uniform_location(SHADER_LIGHT_SPECULAR_NAME), 1,
               course_lights[0].specular);
  glUniform4fv(shader_get_uniform_location(SHADER_LIGHT_DIFFUSE_NAME), 1,
               course_lights[0].diffuse);
  glUniform4fv(shader_get_uniform_location(SHADER_LIGHT_AMBIENT_NAME), 1,
               course_lights[0].ambient);

  glUniform4fv(shader_get_uniform_location(SHADER_FOG_NAME), 1, fog);
}

static int course_light_cb(ClientData cd, Tcl_Interp *ip, int argc,
                           const char **argv) {
  int light_num;
  scalar_t tmp_arr[4];
  double tmp_dbl;
  ...
  while (!error && argc > 0) {
    if (strcmp("-on", *argv) == 0) {
      ...
      for (i = 0; i < 3; i++) {
        magnitude += course_lights[light_num].position[i] *
                     course_lights[light_num].position[i];
      }
      for (i = 0; i < 3; i++) {

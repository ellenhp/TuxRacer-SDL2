...
scalar_t frand() { return (scalar_t)rand() / RAND_MAX; }
...
void create_new_particles(point_t loc, vector_t vel, int num) {
  ...
  for (i = 0; i < num; i++) {
    ...
    newp->vel =
        add_vectors(scale_vector(speed, vel),
...
void update_particles(scalar_t time_step) {
  ...
  for (p = &head; *p != NULL;) {
    ...
    (**p).cur_size = NEW_PART_SIZE + (OLD_PART_SIZE - NEW_PART_SIZE) *
                                         ((**p).age / (**p).death);
...
static int particle_colour_cb(ClientData cd, Tcl_Interp *ip, int argc,
                              const char *argv[]) {
  scalar_t tmp_arr[4];
  bool_t error = False;

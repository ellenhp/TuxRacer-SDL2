...
void adjust_tux_joints(scalar_t turnFact, bool_t isBraking,
  ...
  kick_paddling_angle =
      MAX_KICK_PADDLING_ANGLE * sin(paddling_factor * M_PI * 2.0);

  turning_angle[0] = max(-turnFact, 0.0) * MAX_ARM_ANGLE;
  turning_angle[1] = max(turnFact, 0.0) * MAX_ARM_ANGLE;

  flap_angle =
      MAX_ARM_ANGLE * (0.5 + 0.5 * sin(M_PI * flap_factor * 6 - M_PI / 2));

  /* Adjust arms for turning */
  rotate_scene_node(
      tuxLeftShoulderJoint, 'z',
      min(braking_angle + paddling_angle + turning_angle[0], MAX_ARM_ANGLE) +
          flap_angle);
  rotate_scene_node(
      tuxRightShoulderJoint, 'z',
      min(braking_angle + paddling_angle + turning_angle[1], MAX_ARM_ANGLE) +
          flap_angle);
  ...
  rotate_scene_node(tuxLeftKneeJoint, 'z',
                    -10 + turn_leg_angle - min(35, speed) +
                        kick_paddling_angle + force_angle);
  reset_scene_node(tuxRightKneeJoint);
  rotate_scene_node(tuxRightKneeJoint, 'z',
                    -10 - turn_leg_angle - min(35, speed) -
                        kick_paddling_angle + force_angle);
...
static int head_cb(ClientData cd, Tcl_Interp *ip, int argc,
                   const char *argv[]) {
  if (argc != 2) {
...
static int neck_cb(ClientData cd, Tcl_Interp *ip, int argc,
                   const char *argv[]) {
...
static int root_node_cb(ClientData cd, Tcl_Interp *ip, int argc,
                        const char *argv[]) {
  if (argc != 2) {
    Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n",
...
static int left_shoulder_cb(ClientData cd, Tcl_Interp *ip, int argc,
                            const char *argv[]) {
...
static int right_shoulder_cb(ClientData cd, Tcl_Interp *ip, int argc,
                             const char *argv[]) {
...
static int left_hip_cb(ClientData cd, Tcl_Interp *ip, int argc,
                       const char *argv[]) {
...
static int right_hip_cb(ClientData cd, Tcl_Interp *ip, int argc,
                        const char *argv[]) {
...
static int left_knee_cb(ClientData cd, Tcl_Interp *ip, int argc,
                        const char *argv[]) {
...
static int right_knee_cb(ClientData cd, Tcl_Interp *ip, int argc,
                         const char *argv[]) {
...
static int left_ankle_cb(ClientData cd, Tcl_Interp *ip, int argc,
                         const char *argv[]) {
...
static int right_ankle_cb(ClientData cd, Tcl_Interp *ip, int argc,
                          const char *argv[]) {
...
static int tail_cb(ClientData cd, Tcl_Interp *ip, int argc,
                   const char *argv[]) {

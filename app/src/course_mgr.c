...
static list_t speed_course_list = NULL; /* list of open courses */
static list_t score_course_list = NULL; /* list of open courses */

static char *race_condition_names[RACE_CONDITIONS_NUM_CONDITIONS] = {
    "sunny", "cloudy", "night", "evening"};
...
void init_course_manager() {
  ...
  speed_course_list = create_list();
  score_course_list = create_list();
}
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
list_t get_open_courses_list() { return open_course_list; }

list_t get_speed_courses_list() { return speed_course_list; }

list_t get_score_courses_list() { return score_course_list; }
...
unsigned int get_course_index(const char *course_name) {
  ...
  while (cur_elem != NULL) {
    open_course_data_t *data =
        (open_course_data_t *)get_list_elem_data(cur_elem);
...
bool_t is_course_free(const char *course_name) {
  ...
  return (course_index > 0) &&
         ((COURSE_FREE_MASK >> (course_index - 1)) & 0x1U) != 0;
}
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
list_t get_events_list() { return event_list; }
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
char *get_event_name(event_data_t *event) { return event->name; }
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
list_t get_event_cup_list(event_data_t *event) { return event->cup_list; }
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
char *get_cup_name(cup_data_t *cup) { return cup->name; }
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
list_t get_cup_race_list(cup_data_t *cup) { return cup->race_list; }
...
list_elem_t get_last_complete_cup_for_event(event_data_t *event_data) {
  ...
  if (get_last_completed_cup(g_game.player[local_player()].name,
                             get_event_name(event_data), g_game.difficulty,
                             &last_cup)) {
    cup = get_event_cup_by_name(event_data, last_cup);

    if (cup == NULL) {
      print_warning(IMPORTANT_WARNING, "Couldn't find saved cup `%s'",
                    last_cup);
    }
    return cup;
  } else {
...
/*!
 ...
 \date    Modified: 2000-09-24
 */
int compare_race_positions(cup_data_t *cup_data, list_elem_t race1,
                           list_elem_t race2) {
  int incr = 1;
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
static int open_courses_cb(ClientData cd, Tcl_Interp *ip, int argc,
                           const char **argv) {
  char *err_msg;
  ...
  for (i = 0; i < num_courses; i++) {
...
#else
    sprintf(preview_file, "%s/courses/%s/preview.jpg", getparam_data_dir(),
            data->course);
#endif
    ...
    last_elem =
        insert_list_elem(open_course_list, last_elem, (list_elem_data_t)data);
    if (data->speed) {
      last_speed_elem = insert_list_elem(speed_course_list, last_speed_elem,
                                         (list_elem_data_t)data);
    }
    if (data->score) {
      last_score_elem = insert_list_elem(score_course_list, last_score_elem,
                                         (list_elem_data_t)data);
    }
  }
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
static race_data_t *create_race_data(Tcl_Interp *ip, const char *string,
                                     char **err_msg) {
  const char **argv = NULL;
  const char **orig_argv = NULL;
  int argc = 0;
  ...
  while (*argv != NULL) {
    if (strcmp(*argv, "-course") == 0) {
      ...
      if (get_tcl_int_tuple(ip, *argv, herring_req,
                            sizeof(herring_req) / sizeof(herring_req[0])) ==
          TCL_ERROR) {
      ...
      if (get_tcl_tuple(ip, *argv, time_req,
                        sizeof(time_req) / sizeof(time_req[0])) == TCL_ERROR) {
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
static cup_data_t *create_cup_data(Tcl_Interp *ip, const char *string,
                                   char **err_msg) {
  const char **argv = NULL;
  const char **orig_argv = NULL;
  int argc = 0;
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
static event_data_t *create_event_data(Tcl_Interp *ip, const char *string,
                                       char **err_msg) {
  const char **orig_argv = NULL;
  const char **argv = NULL;
  int argc = 0;
  ...
  while (*argv != NULL) {
    if (strcmp(*argv, "-name") == 0) {
      ...
      for (i = 0; i < num_cups; i++) {
        ...
        last_cup =
            insert_list_elem(cup_list, last_cup, (list_elem_data_t)cup_data);
      }
...
/*!
 ...
 \date    Modified: 2000-09-19
 */
static int events_cb(ClientData cd, Tcl_Interp *ip, int argc,
                     const char **argv) {
  char *err_msg;
  const char **list = NULL;
  int num_events;
  ...
  for (i = 0; i < num_events; i++) {
    ...
    last_event =
        insert_list_elem(event_list, last_event, (list_elem_data_t)data);
  }
 ...
 */
static int get_race_conditions_cb(ClientData cd, Tcl_Interp *ip, int argc,
                                  const char **argv) {
  char *err_msg;
  Tcl_Obj *result;
  ...
  result =
      Tcl_NewStringObj(race_condition_names[g_game.race.conditions],
...
void register_course_manager_callbacks(Tcl_Interp *ip) {
  ...
  Tcl_CreateCommand(ip, "tux_get_race_conditions", get_race_conditions_cb, 0,
                    0);
}

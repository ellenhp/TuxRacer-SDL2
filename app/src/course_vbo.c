...
#define TERRAIN_AT_COORD ((GLubyte *)(DATA_AT_COORD + 8))
  ...
  glBufferData(GL_ARRAY_BUFFER, nx * nz * STRIDE_BYTES, VNT_array,
               GL_STATIC_DRAW);
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, num_indices * sizeof(GLfloat), indices,
               GL_STATIC_DRAW);
...
int get_num_vertices(int startZ, int *nextStartZ) {
  ...
  for (z = startZ; z < course_z_size - 1; z++) {
    if (indices + 6 * course_x_size >= USHRT_MAX ||
        ((GLfloat)(z - get_start_z()) / (course_z_size - 1.) * course_length) >
            getparam_forward_clip_distance() / 2) {
      break;
    }
    for (x = 0; x < course_x_size - 1; x++) {
      indices += 6;
...
void draw_course_vbo() {
  ...
  while (numVerticies = get_num_vertices(startZ, &nextStartZ)) {
    glVertexAttribPointer(shader_get_attrib_location(SHADER_VERTEX_NAME), 3,
                          GL_FLOAT, GL_FALSE, STRIDE_BYTES,
                          startZ * course_x_size * STRIDE_BYTES);
    glEnableVertexAttribArray(shader_get_attrib_location(SHADER_VERTEX_NAME));

    glVertexAttribPointer(shader_get_attrib_location(SHADER_NORMAL_NAME), 3,
                          GL_FLOAT, GL_FALSE, STRIDE_BYTES,
                          startZ * course_x_size * STRIDE_BYTES +
                              3 * sizeof(GLfloat));
    ...
    glVertexAttribPointer(shader_get_attrib_location(SHADER_TEXTURE_COORD_NAME),
                          2, GL_FLOAT, GL_FALSE, STRIDE_BYTES,
                          startZ * course_x_size * STRIDE_BYTES +
                              6 * sizeof(GLfloat));
    glEnableVertexAttribArray(
        shader_get_attrib_location(SHADER_TEXTURE_COORD_NAME));

    glVertexAttribPointer(shader_get_attrib_location(SHADER_TERRAINS_NAME), 3,
                          GL_UNSIGNED_BYTE, GL_FALSE, STRIDE_BYTES,
                          startZ * course_x_size * STRIDE_BYTES +
                              8 * sizeof(GLfloat));

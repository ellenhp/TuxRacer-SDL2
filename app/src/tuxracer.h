...
#if defined(WIN32) || defined(__CYGWIN__) || defined(NATIVE_WIN32_COMPILER)
#ifndef WIN32
#define WIN32
#endif
#include <windows.h>
...
/* Map Open GLES name to "regular" Open GL */
...
#else
#error                                                                         \
    "You don't have finite(), _finite(), isnan(), or _isnan() on your system!"
#endif
...
/* Multiplayer is not yet supported: currently used for "speedMode" */
#ifdef SPEED_MODE
...
/* View mode */
typedef enum { BEHIND, FOLLOW, ABOVE, TUXEYE, NUM_VIEW_MODES } view_mode_t;
...
/* Control mode */
typedef enum { KEYBOARD = 0, MOUSE = 1, JOYSTICK = 2 } control_mode_t;
...
typedef struct {
  ...
  bool_t is_accelerating;  /* is player accelerating? (beause paddling is not
                              continue */
...
typedef struct {
  ...
  bool_t practicing; /* are we in practice mode (wich is now the world challenge
                        mode)? Or in training mode ? */
  bool_t race_aborted; /* was the race quit prematurely? */
  bool_t race_paused;
  bool_t needs_save_or_display_rankings; /* when game is over, before returning
                                            to race select, check this */
  bool_t rankings_displayed; /* when game is over, before returning to race
                                select, check this */
  bool_t race_time_over; /* was the race quit prematurely because remaining time
                            became NULL ? */
#ifdef SPEED_MODE
...
#ifdef SPEED_MODE
#define game_has_herring() (!g_game.is_speed_only_mode || g_game.practicing)
#else
#define game_has_herring() (g_game.practicing)
#endif

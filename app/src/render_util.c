...
__inline GLfixed FixedFromFloat(float value) { return (GLfixed)value; };
__inline GLfixed MultiplyFixed(GLfixed op1, GLfixed op2) {
  return (op1 * op2) >> PRECISION;
};

void glesPerspective(GLfloat fovy, GLfloat aspect, GLfloat zNear,
                     GLfloat zFar) {
...
void clear_rendering_context() {
  glDepthMask(GL_TRUE);
  glClearColor(ui_background_colour.r, ui_background_colour.g,
               ui_background_colour.b, ui_background_colour.a);
  glClearStencil(0);
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
}

void draw_billboard(player_data_t *plyr, point_t center_pt, scalar_t width,
                    scalar_t height, bool_t use_world_y_axis,

...
int winsys_joystick_x_trigger_time = INT_MAX,
...
static Uint32 sdl_flags = SDL_INIT_VIDEO | SDL_INIT_NOPARACHUTE |
...
#if SDL_MAJOR_VERSION == 2
  if ((window = SDL_CreateWindow("Tux Racer", SDL_WINDOWPOS_UNDEFINED,
...
float winsys_scale(float x) { return x * scale_factor; }
...
void winsys_scan_joysticks() {
  ...
  for (controllerIndex = 0; controllerIndex < num_joysticks;
       controllerIndex++) {
    ...
    SDL_JoystickGetGUIDString(SDL_JoystickGetGUID(winsys_joystick), guid,
                              sizeof(guid));

    winsys_num_buttons = SDL_JoystickNumButtons(winsys_joystick);
    print_debug(DEBUG_JOYSTICK, "Joystick has %d button%s", winsys_num_buttons,
                winsys_num_buttons == 1 ? "" : "s");

    /* Get number of axes */
    winsys_num_axes = SDL_JoystickNumAxes(winsys_joystick);
    print_debug(DEBUG_JOYSTICK, "Joystick has %d ax%ss", winsys_num_axes,
                winsys_num_axes == 1 ? "i" : "e");
    ...
    if (winsys_num_axes == 3) {
      ...
      // print_debug(DEBUG_JOYSTICK, "Incompatible joystick '%s' with GUID
      // '%s'", js_name, guid); print_debug(DEBUG_JOYSTICK, "Creating best-guess
      // mapping for joystick '%s' with GUID '%s'", js_name, guid);
      // sprintf(mapping_buffer, "%s,Unrecognized
      // Joystick,a:b5,b:b6,x:b8,y:b9,dpup:b0,dpleft:b2,dpdown:b1,dpright:b3,leftx:a0,lefty:a1,");
...
void winsys_init_joystick() {
  ...
  SDL_GameControllerAddMapping(
      "8e06f600000000000000504944564944,CH FLIGHTSTICK "
      "PRO,a:b1,b:b3,leftx:a0,lefty:a1,x:b0,y:b2,");
  SDL_GameControllerAddMapping(
      "8e06f400000000000000504944564944,CH "
      "COMBATSTICK,a:b6,b:b5,y:b4,x:b7,start:b9,dpup:h0.1,dpleft:h0.8,dpdown:"
      "h0.4,dpright:h0.2,leftx:a0,lefty:a1,");
  ...
  while (True) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_JOYAXISMOTION:
        if (!winsys_game_controller &&
            event.jaxis.axis == getparam_joystick_x_axis()) {
          // accelerometer
      ...
      case SDL_CONTROLLERBUTTONDOWN:
        ...
        for (i = 0; i < num_js_bindings; i++) {
          if (js_bindings[i].js_button == event.cbutton.button) {
            keyboard_func(js_bindings[i].sdl_key, js_bindings[i].sdl_key > 255,
                          False, 0, 0);
          }
        }
        break;
      ...
      case SDL_MOUSEBUTTONUP:
        ...
        if (mouse_func) {
          (*mouse_func)(event.button.button, event.button.state, -1,
      ...
      case SDL_FINGERDOWN:
        ...
        if (mouse_func) {
          (*mouse_func)(SDL_BUTTON_LEFT, SDL_PRESSED, event.tfinger.fingerId,
      ...
      case SDL_FINGERUP:
        ...
        if (mouse_func) {
          (*mouse_func)(SDL_BUTTON_LEFT, SDL_RELEASED, event.tfinger.fingerId,
                        (int)(event.tfinger.x * getparam_x_resolution()),
                        (int)(event.tfinger.y * getparam_y_resolution()));
        }
        break;
...
/*!
  ...
  \date    Created:  2013-10-08
  \date    Modified: 2013-10-08 */
void winsys_add_js_axis_bindings() { bind_js_axes = True; }
...
void winsys_update_joysticks() {
  ...
  if (winsys_joystick_x_status != 0) {
    ...
    } else if (winsys_joystick_x_repeating &&
               SDL_GetTicks() - winsys_joystick_x_trigger_time >
                   JOYSTICK_REPEAT_RATE_INVERSE) {
    ...
    } else if (SDL_GetTicks() - winsys_joystick_x_trigger_time >
               JOYSTICK_REPEAT_DELAY) {
  ...
  if (winsys_joystick_y_status != 0) {
    if (fabs(y_joystick) < JOYSTICK_RELAX) {
      ...
      winsys_joystick_y_trigger_time = INT_MAX;
    } else if (winsys_joystick_y_repeating &&
               SDL_GetTicks() - winsys_joystick_y_trigger_time >
                   JOYSTICK_REPEAT_RATE_INVERSE) {
    ...
    } else if (SDL_GetTicks() - winsys_joystick_y_trigger_time >
               JOYSTICK_REPEAT_DELAY) {
...
/*!
  ...
  \date    Modified: 2000-10-19
*/
void winsys_set_idle_func(winsys_idle_func_t func) { glutIdleFunc(func); }
...
/*!
  ...
  \date    Modified: 2000-10-19
*/
void winsys_set_mouse_func(winsys_mouse_func_t func) { glutMouseFunc(func); }
...
/*!
  ...
  \date    Modified: 2000-10-19
*/
void winsys_set_motion_func(winsys_motion_func_t func) { glutMotionFunc(func); }
...
/*!
  ...
  \date    Modified: 2000-10-19
*/
void winsys_swap_buffers() { glutSwapBuffers(); }
...
/*!
  ...
  \date    Modified: 2000-10-19
*/
void winsys_warp_pointer(int x, int y) { glutWarpPointer(x, y); }
...
/*!
  ...
  \date    Modified: 2000-10-19
*/
void winsys_enable_key_repeat(bool_t enabled) { glutIgnoreKeyRepeat(!enabled); }

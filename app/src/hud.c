...
static void draw_time(player_data_t *plyr) {
  ...
  scalar_t time;
  // depending of the calculation mode, we want to display either the time
  // spendt or the time remaining Half_Pipe mode
  if (!strcmp(get_calculation_mode(), "Half_Pipe")) {
    ...
    time = (plyr->control.is_flying)
               ? (g_game.time - plyr->control.fly_start_time +
  ...
  y_org = getparam_y_resolution() - OVERSCAN_MARGIN_Y - TIME_Y_OFFSET -
          TIME_ICON_SIZE;
...
#define TO_RELATIVE(screen, val) (((val) / screen * 2.0) - 1.0)
    ...
    GLfloat vertices[] = {x_org,
                          y_org,
                          0,
                          x_org + TIME_ICON_SIZE,
                          y_org,
                          0,
                          x_org + TIME_ICON_SIZE,
                          y_org + TIME_ICON_SIZE,
                          0,
                          x_org,
                          y_org + TIME_ICON_SIZE,
                          0};
    ...
    for (i = 0; i < sizeof(vertices) / sizeof(vertices[0]); i += 3) {
      vertices[i] =
          TO_RELATIVE(screen_w, vertices[i] + getparam_x_resolution() -
                                    HERRING_ICON_X_OFFSET - OVERSCAN_MARGIN_X);
      vertices[i + 1] = TO_RELATIVE(
          screen_h, vertices[i + 1] + getparam_y_resolution() -
                        HERRING_ICON_Y_OFFSET - OVERSCAN_MARGIN_Y - asc);
    }
...
#undef TO_RELATIVE
  ...
  big_w = w;
  draw_string(font, string, TIME_X_OFFSET + OVERSCAN_MARGIN_X + TIME_ICON_SIZE,
  ...
  draw_string(font, string,
              TIME_X_OFFSET + OVERSCAN_MARGIN_X + big_w + 5 + TIME_ICON_SIZE,
              time_y_refval - TIME_Y_OFFSET - OVERSCAN_MARGIN_Y - asc - 2);
}
...
#define TO_RELATIVE(screen, val) (((val) / screen * 2.0) - 1.0)
  ...
  if (!get_texture_binding(binding, &texobj)) {
    print_warning(IMPORTANT_WARNING, "Couldn't get texture for binding %s",
                  binding);
    return;
  }
...

  {
    ...
    for (i = 0; i < sizeof(verticesItem) / sizeof(verticesItem[0]); i += 3) {
      verticesItem[i] =
          TO_RELATIVE(screen_w, verticesItem[i] + getparam_x_resolution() -
                                    HERRING_ICON_X_OFFSET - OVERSCAN_MARGIN_X);
      verticesItem[i + 1] = TO_RELATIVE(
          screen_h, verticesItem[i + 1] + getparam_y_resolution() -
                        HERRING_ICON_Y_OFFSET - OVERSCAN_MARGIN_Y - asc);
    }

    glVertexAttribPointer(shader_get_attrib_location(SHADER_VERTEX_NAME), 3,
                          GL_FLOAT, GL_FALSE, 0, verticesItem);
    ...
    glVertexAttribPointer(shader_get_attrib_location(SHADER_TEXTURE_COORD_NAME),
                          2, GL_FLOAT, GL_FALSE, 0, texCoordsItem);
  ...
  draw_string(font, string,
              getparam_x_resolution() - HERRING_ICON_X_OFFSET -
                  OVERSCAN_MARGIN_X + HERRING_ICON_WIDTH,
              getparam_y_resolution() - HERRING_ICON_Y_OFFSET -
                  OVERSCAN_MARGIN_Y - asc + HERRING_COUNT_Y_OFFSET);
...
static void draw_score(player_data_t *plyr) {
  ...
  draw_string(font, string, SCORE_X_OFFSET + OVERSCAN_MARGIN_X,
  ...
  draw_string(font, string, SCORE_X_OFFSET + OVERSCAN_MARGIN_X + w,
              SCORE_Y_OFFSET + OVERSCAN_MARGIN_Y);
}
...
point2d_t calc_new_fan_pt(scalar_t angle) {
  point2d_t pt;
  pt.x = ENERGY_GAUGE_CENTER_X +
         cos(ANGLES_TO_RADIANS(angle)) * SPEEDBAR_OUTER_RADIUS;
  pt.y = ENERGY_GAUGE_CENTER_Y +
         sin(ANGLES_TO_RADIANS(angle)) * SPEEDBAR_OUTER_RADIUS;
...
point2d_t calc_new_fan_tex_pt(scalar_t angle) {
  point2d_t pt;
  pt.x = (cos(ANGLES_TO_RADIANS(angle)) * SPEEDBAR_OUTER_RADIUS +
          ENERGY_GAUGE_CENTER_X) /
         GAUGE_IMG_SIZE;
  pt.y = (sin(ANGLES_TO_RADIANS(angle)) * SPEEDBAR_OUTER_RADIUS +
...
void draw_partial_tri_fan(scalar_t fraction, scalar_t center_x,
                          scalar_t center_y) {
...
#define TO_RELATIVE(screen, val) (((val) / screen * 2.0) - 1.0)
  ...
  angle_incr =
      fraction * (SPEEDBAR_BASE_ANGLE - SPEEDBAR_MAX_ANGLE) / CIRCLE_DIVISIONS;
  ...
  vertices[sizeof(vertices) / sizeof(GLfloat) - 3] =
      TO_RELATIVE(screen_w, pt.x + center_x);
  vertices[sizeof(vertices) / sizeof(GLfloat) - 2] =
      TO_RELATIVE(screen_h, pt.y + center_y);
...
#define TO_RELATIVE(screen, val) (((val) / screen * 2.0) - 1.0)
...

  {
    ...
    for (i = 0; i < sizeof(verticesItem) / sizeof(verticesItem[0]); i += 3) {
      verticesItem[i] =
          TO_RELATIVE(screen_w, verticesItem[i] + getparam_x_resolution() -
                                    GAUGE_WIDTH - OVERSCAN_MARGIN_X);
      verticesItem[i + 1] =
          TO_RELATIVE(screen_h, verticesItem[i + 1] + OVERSCAN_MARGIN_Y);
    }
    ...
    glVertexAttribPointer(shader_get_attrib_location(SHADER_VERTEX_NAME), 3,
                          GL_FLOAT, GL_FALSE, 0, verticesItem + 36);
    glVertexAttribPointer(shader_get_attrib_location(SHADER_TEXTURE_COORD_NAME),
                          2, GL_FLOAT, GL_FALSE, 0, texcoordItem);
    ...
    glVertexAttribPointer(shader_get_attrib_location(SHADER_VERTEX_NAME), 3,
                          GL_FLOAT, GL_FALSE, 0, verticesItem + 18);
    glVertexAttribPointer(shader_get_attrib_location(SHADER_TEXTURE_COORD_NAME),
                          2, GL_FLOAT, GL_FALSE, 0, texcoordItem + 12);
    glDrawArrays(GL_TRIANGLES, 0, 6);
    ...
    if (speed > SPEEDBAR_GREEN_MAX_SPEED) {
      ...
      speedbar_frac +=
          speed / SPEEDBAR_GREEN_MAX_SPEED * SPEEDBAR_GREEN_FRACTION;
    }

    glBindTexture(GL_TEXTURE_2D, speedmask_texobj);

    shader_set_color(speedbar_background_color);
    draw_partial_tri_fan(
        1.0, getparam_x_resolution() - GAUGE_WIDTH - OVERSCAN_MARGIN_X,
        OVERSCAN_MARGIN_Y);
    shader_set_color(white);
    draw_partial_tri_fan(min(1.0, speedbar_frac),
                         getparam_x_resolution() - GAUGE_WIDTH -
                             OVERSCAN_MARGIN_X,
                         OVERSCAN_MARGIN_Y);
    ...
    draw_string(speed_font, string,
                getparam_x_resolution() - GAUGE_WIDTH - OVERSCAN_MARGIN_X +
                    ENERGY_GAUGE_CENTER_X - w / 2.0,
    ...
    draw_string(units_font, string,
                getparam_x_resolution() - GAUGE_WIDTH - OVERSCAN_MARGIN_X +
                    ENERGY_GAUGE_CENTER_X - w / 2.0,
                ENERGY_GAUGE_BOTTOM + ENERGY_GAUGE_HEIGHT / 2.0 - asc -
                    SPEED_UNITS_Y_OFFSET + OVERSCAN_MARGIN_Y);
  }
#undef TO_RELATIVE
}
void print_fps() {
  ...
  draw_string(font, "FPS: ", FPS_X_OFFSET + OVERSCAN_MARGIN_X,
              FPS_Y_OFFSET + OVERSCAN_MARGIN_Y + y);

  draw_string(font_big, string, FPS_X_OFFSET + OVERSCAN_MARGIN_X + x,
              FPS_Y_OFFSET + OVERSCAN_MARGIN_Y + y);
}

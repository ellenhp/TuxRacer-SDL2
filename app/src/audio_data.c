...
static bool_t load_sound_or_music(char *name, char *filename,
  ...
  if (data_ptr == NULL) {
    print_warning(SOUNDFILE_MISSING_WARNING_LEVEL,
                  "FAILED to load %s file %s: %s", desc, filename,
                  Mix_GetError());
    return False;
  }
...
void incr_sound_data_ref_ctr(char *name) {
  ...
  print_debug(DEBUG_SOUND,
              "incremented reference counter of sound %s "
              "to %d",
              name, srec->ref_ctr);
}
...
void decr_sound_data_ref_ctr(char *name) {
  ...
  print_debug(DEBUG_SOUND,
              "decremented reference counter of sound %s "
              "to %d",
              name, srec->ref_ctr);
}
...
void incr_music_data_ref_ctr(char *name) {
  ...
  print_debug(DEBUG_SOUND,
              "incremented reference counter of music %s "
              "to %d",
              name, mrec->ref_ctr);
}
...
void decr_music_data_ref_ctr(char *name) {
  ...
  print_debug(DEBUG_SOUND,
              "decremented reference counter of music %s "
              "to %d",
              name, mrec->ref_ctr);
}
...
*/
static int load_sound_cb(ClientData cd, Tcl_Interp *ip, int argc,
                         const char *argv[]) {
  Tcl_Obj *result;
...
/* Name:          load_music_cb
   ...
   Last Modified: 2000-09-02
*/
static int load_music_cb(ClientData cd, Tcl_Interp *ip, int argc,
                         char *argv[]) {
  Tcl_Obj *result;
...
bool_t load_sound(char *name, char *filename) { return True; }

bool_t load_music(char *name, char *filename) { return True; }

bool_t get_sound_data(char *name, Mix_Chunk **data) { return False; }

bool_t get_music_data(char *name, Mix_Music **data) { return False; }

void set_music_playing_status(char *name, bool_t playing) {}

bool_t get_music_playing_status(char *name) { return False; }
...
bool_t is_sound_data_dirty() { return False; }

bool_t is_music_data_dirty() { return False; }

...
#define TO_RELATIVE(screen, val) (((val) / screen * 2.0) - 1.0)
  {
    GLfloat vertices[] = {
        -0.6,
        TO_RELATIVE(height,
                    (y_base - (line - 2) * (asc + desc)) - winsys_scale(5.0)),
        0,
        -0.6,
        TO_RELATIVE(height,
                    (y_base - (line - 1) * (asc + desc)) - winsys_scale(5.0)),
        0,
        0.6,
        TO_RELATIVE(height,
                    (y_base - (line - 1) * (asc + desc)) - winsys_scale(5.0)),
  ...
  draw_string(font, (char *)string, width / 2 - w / 2,
...
static void drawRedCircle(GLint x, GLint y, GLint diameter) {
  ...
  {
    GLfloat texcoords[] = {tll.x, tll.y, tll.x, tur.y,
                           tur.x, tur.y, tur.x, tll.y};
...
bool_t game_abort_is_for_tutorial(void) { return training_abort; }
...
static void draw_instructions(player_data_t *plyr) {
  ...
  case 1:
    ...
    if (plyr->control.turn_fact > 0.5)
      step++;
    break;
  case 2:
    print_instruction(Localize("Now make tux turn left.", ""), 1);
    if (plyr->control.turn_fact < -0.4)
      step++;
    break;
  case 3:
    ...
    if (!winsys_is_controller_active()) {
      ...
      print_instruction(
          Localize("(Tap the bottom right corner of the screen)", ""), 2);
      drawRedCircle(getparam_x_resolution() - radius * 2, 0, radius * 2);
    } else {
      print_instruction(Localize("(Push the analog stick forward)", ""), 2);
    }
    if (check_condition_for_time(plyr->control.is_accelerating, 1000))
      step++;
    break;
  case 4:
    print_instruction(
        Localize("Braking lets tux turn harder at high speed.", ""), 1);
    if (!winsys_is_controller_active()) {
      const GLint radius = winsys_scale(55);
      print_instruction(Localize("Tap the bottom left corner to brake.", ""),
                        2);
      drawRedCircle(0, 0, radius * 2);
    } else {
      print_instruction(Localize("Pull the analog stick back to brake.", ""),
                        2);
    }
  ...
  case 5:
    ...
    if (check_condition_for_time(True, 2000))
      step++;
    break;
  case 6:
    if (!winsys_is_controller_active()) {
    ...
    if (g_game.race_paused == True)
      step++;
    break;
  case 7:
    print_instruction(Localize("Cool! Now come back to game.", ""), 0);

    if (g_game.race_paused == False)
      step = -1;
    break;
  case -1:
    training_pause_for_tutorial_explanation();
    print_instruction(
        Localize("Good job. Now you know the basics of racing", ""), 0);
    print_instruction(
        Localize("The next tutorial will teach you how to jump", ""), 1);
  ...
  case 11:
    if (!winsys_is_controller_active()) {
      const GLint radius = winsys_scale(55);
      print_instruction(Localize("Push the red area to accumulate enough", ""),
                        1);
      print_instruction(Localize("energy to jump.", ""), 2);
      drawRedCircle(0, getparam_y_resolution() - radius * 2, radius * 2);
    } else {
      print_instruction(
          Localize("Hold down \x01 to accumulate enough energy", ""), 1);
    ...
    break;
  case 12:
    print_instruction(Localize("You can see the energy gauge filling...", ""),
                      1);
  ...
  case 14:
    ...
    print_instruction(Localize("if you can spend a whole second in the", ""),
                      2);
    print_instruction(Localize("air. Use the terrain to your advantage.", ""),
                      3);
  ...
  case 15:
    print_instruction(
        Localize("Try to do a longer jump (>1sec) off a bump.", ""), -3);
  ...
  case 17:
    training_pause_for_tutorial_explanation();
    print_instruction(
        Localize("There are two main things you can do while", ""), 0);
    print_instruction(Localize("you're in the air. First, you can try and", ""),
                      1);
    print_instruction(
        Localize("do a cool trick. Second, you can accelerate", ""), 2);
  ...
  case 18:
    training_pause_for_tutorial_explanation();
    print_instruction(
        Localize("To do a trick, find a bump while you're going", ""), 1);
    print_instruction(
        Localize("fast and jump off of it. While you're in the air,", ""), 2);
    if (!winsys_is_controller_active()) {
      const GLint radius = winsys_scale(55);
      print_instruction(
          Localize("tap the upper right corner of the screen.", ""), 3);
      drawRedCircle(getparam_x_resolution() - radius * 2,
                    getparam_y_resolution() - radius * 2, radius * 2);
    }
    plyr->tricks = 0;
    if (training_is_resumed())
      step++;
  ...
  case 21:
    ...
    print_instruction(Localize("while flapping your wings to go faster.", ""),
                      2);
    if (training_is_resumed()) {
      step++;
      check_condition_for_time(False, 0);
      /* This code resets the player to somewhere else on the map. Why was this
      put here??? if ((plyr->pos.z)<(-200)) { point_t p =
      make_point(48.0,-105.8,-200.0); racing_init_for_tutorial(p);
      }
                      */
    }
    break;
  case 22:
    print_instruction(Localize("Try to do a long flying jump (>1sec).", ""),
                      -3);
    if (winsys_is_controller_active()) {
      print_instruction(
          Localize("Make sure you're pushing the analog stick.", ""), -2);
    } else {
      print_instruction(
          Localize("Make sure you're tapping the lower right.", ""), -2);
    }
    if (check_condition_for_time(
            (bool_t)(plyr->control.is_flying && plyr->control.is_accelerating),
            1000))
      step = -2;
    break;
  case -2:
    print_instruction(
        Localize("Congratulations, you finished this tutorial.", ""), -1);
    print_instruction(Localize("You can apply these skills to different", ""),
                      0);
    print_instruction(
        Localize("courses. Sometimes you should focus on speed", ""), 1);
    print_instruction(
        Localize("and picking up food. Sometimes you have to", ""), 2);

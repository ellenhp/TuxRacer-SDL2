...
typedef enum { SCORE_INFO, NUM_SCORE_DATA_TYPES } score_data_t;
...
typedef struct {
  ...
  int score;
  char future_use[STANDARD_RECORD_SIZE - sizeof(single_event_t) - sizeof(int) -
...
void write_high_scores(void) {
  ...
  if (get_high_score_file_name(score_file, sizeof(score_file)) == 0) {
    ...
    while (next_hash_entry(score_scan_ptr, &event_name,
                           (hash_entry_t *)&cup_table)) {
      begin_hash_scan(*cup_table, &cup_scan_ptr);
      while (next_hash_entry(cup_scan_ptr, &cup_name,
                             (hash_entry_t *)&this_score)) {
...
void init_saved_games(void) {
  ...
  for (cur_dir_file = get_list_head(dir_file_list); cur_dir_file != NULL;
    ...
    if (get_sav_index(cur_dir_filename, &sav_index)) {
      ...
      if (SDL_RWread(save_stream, magic, sizeof(magic), 1) != 1 ||
        ...
        print_warning(IMPORTANT_WARNING, "`%s' is not a valid saved game file",
                      file_name);
      ...
      if (save_stream != NULL) {
        while (SDL_RWread(save_stream, &this_save, sizeof(this_save), 1)) {
          ...
          case EVENT_INFO:
            set_last_completed_cup(player_name, this_save.data.event.event,
            ...
            print_debug(
                ...
                cur_dir_filename, player_name, this_save.data.event.event,
                this_save.data.event.difficulty, this_save.data.event.cup);
            break;

          case RACE_RESULTS:
            set_saved_race_results(
                player_name, this_save.data.results.event,
                this_save.data.results.cup, this_save.data.results.race,
                this_save.data.results.difficulty, this_save.data.results.time,
                this_save.data.results.herring, this_save.data.results.score);
            print_debug(
                DEBUG_SAVE,
                "Read results from `%s': "
                ...
                "herring: %d, score: %d",
                cur_dir_filename, player_name, this_save.data.results.event,
                this_save.data.results.cup, this_save.data.results.race,
                this_save.data.results.difficulty, this_save.data.results.time,
                this_save.data.results.herring, this_save.data.results.score);
            break;
        ...
        print_warning(IMPORTANT_WARNING, "Couldn't read file `%s': %s",
...
bool_t get_last_completed_cup(char *player, char *event, difficulty_level_t d,
                              char **cup) {
  hash_table_t event_table;
  save_info_t *this_save;

  if (get_hash_entry(progress_save_table, player,
...
bool_t set_last_completed_cup(char *player, char *event, difficulty_level_t d,
                              char *cup) {
  hash_table_t event_table;
  save_info_t *this_save;
  ...
  if (!get_hash_entry(event_table, event, (hash_entry_t *)&this_save)) {
    this_save =
        (save_info_t *)malloc(sizeof(save_info_t) * DIFFICULTY_NUM_LEVELS);
...
bool_t get_saved_race_results(char *player, char *event, char *cup, char *race,
                              difficulty_level_t d, scalar_t *time,
...
bool_t set_saved_race_results(char *player, char *event, char *cup, char *race,
                              difficulty_level_t d, scalar_t time, int herring,
                              int score) {
  hash_table_t player_table;
  hash_table_t event_table;
  hash_table_t cup_table;
...
static void truncate_and_init_save_file(char *player_name) {
  ...
  if (get_save_game_file_name(save_file, player_name, sizeof(save_file)) == 0) {
    ...
    if (save_stream == NULL) {
      print_warning(IMPORTANT_WARNING, "Couldn't open `%s' for writing: %s",
      ...
      if (SDL_RWclose(save_stream) != 0) {
        print_warning(IMPORTANT_WARNING, "Couldn't close `%s': %s", save_file,
                      strerror(errno));
      }
    }
  } else {
    print_warning(IMPORTANT_WARNING,
                  "Couldn't get save game file for player `%s'", player_name);
  }
}
...
void write_saved_games(void) {
  ...
  while (next_hash_entry(player_scan_ptr, &player_name,
                         (hash_entry_t *)&event_table)) {
    if (get_save_game_file_name(save_file, player_name, sizeof(save_file)) ==
        0) {
        ...
        while (next_hash_entry(save_scan_ptr, &event_name,
          ...
          for (i = 0; i < DIFFICULTY_NUM_LEVELS; i++) {
            ...
            if (this_save[level].data_type >= 0) {
              SDL_RWwrite(save_stream, &(this_save[level]), 1,
...
char *editSynchronizeScoresRequest() {
  ...
  for (p = 0; p < g_game.num_players; p++) {
    scalar_t time;
    int herring;
    int score;
    int minutes;
    ...
    do {
      ...
      if (get_saved_race_results(plyr->name, g_game.current_event,
                                 g_game.current_cup, data->name,
                                 g_game.difficulty, &time, &herring, &score)) {
        int length;
        if (i > 0)
          strcat(request, "&");
        get_time_components(time, &minutes, &seconds, &hundredths);
        length = strlen(request);
        sprintf(request + length,
                "piste[%d]=%s&score[%d]=%d&herring[%d]=%d&time[%d]=%02d:%02d:%"
                "02d&order[%d]=%s",
                i, data->name, i, score, i, herring, i, minutes, seconds,
                hundredths, i, order);
        i++;
      }

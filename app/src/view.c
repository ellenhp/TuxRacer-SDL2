...
view_mode_t get_view_mode(player_data_t *plyr) { return plyr->view.mode; }
...
point_t interpolate_view_pos(point_t plyr_pos1, point_t plyr_pos2,
                             scalar_t max_vec_angle, point_t pos1, point_t pos2,
...
/*!
  ...
  \date    Modified: 2000-08-26
*/
void interpolate_view_frame(vector_t up1, vector_t dir1, vector_t *p_up2,
                            vector_t *p_dir2, scalar_t dt,
                            scalar_t time_constant) {
  quaternion_t q1, q2;
  scalar_t alpha;
  vector_t x1, y1, z1;
  ...
  make_change_of_basis_matrix(cob_mat1, inv_cob_mat1, x1, y1, z1);
  ...
  make_change_of_basis_matrix(cob_mat2, inv_cob_mat2, x2, y2, z2);
...
void update_view(player_data_t *plyr, scalar_t dt) {
  ...
  time_constant_mult =
      1.0 / min(1.0, max(0.0, (speed - NO_INTERPOLATION_SPEED) /
                                  (BASELINE_INTERPOLATION_SPEED -
  ...
  case TUXEYE: {
    ...
    if (plyr->control.flip_factor || plyr->control.barrel_roll_factor) {
      ...
      scalar_t n =
          sqrt(plyr->viewdir_for_tuxeye.x * plyr->viewdir_for_tuxeye.x +
               plyr->viewdir_for_tuxeye.y * plyr->viewdir_for_tuxeye.y +
               plyr->viewdir_for_tuxeye.z * plyr->viewdir_for_tuxeye.z);
      plyr->viewdir_for_tuxeye.x /= n;
      plyr->viewdir_for_tuxeye.y /= n;
      plyr->viewdir_for_tuxeye.z /= n;
      n = sqrt(plyr->updir_for_tuxeye.x * plyr->updir_for_tuxeye.x +
               plyr->updir_for_tuxeye.y * plyr->updir_for_tuxeye.y +
               plyr->updir_for_tuxeye.z * plyr->updir_for_tuxeye.z);
      ...
      make_rotation_about_vector_matrix(
          mat1, right, jump_from_time(plyr->control.flip_factor) * 360);
      make_rotation_about_vector_matrix(
          mat, plyr->viewdir_for_tuxeye,
  ...
  case BEHIND: {
    ...
    view_vec = make_vector(
        0,
        sin(ANGLES_TO_RADIANS(course_angle - CAMERA_ANGLE_ABOVE_SLOPE +
                              PLAYER_ANGLE_IN_CAMERA)),
        cos(ANGLES_TO_RADIANS(course_angle - CAMERA_ANGLE_ABOVE_SLOPE +
                              PLAYER_ANGLE_IN_CAMERA)));
    ...
    if (plyr->view.initialized) {
      ...
      int i;
      for (i = 0; i < 2; i++) {
        view_pt = interpolate_view_pos(
            plyr->pos, plyr->pos, MAX_CAMERA_PITCH, plyr->view.pos, view_pt,
            CAMERA_DISTANCE, dt,
  ...
  case FOLLOW: {
    ...
    if (plyr->view.initialized) {
      ...
      for (i = 0; i < 2; i++) {
        view_pt = interpolate_view_pos(
            plyr->view.plyr_pos, plyr->pos, MAX_CAMERA_PITCH, plyr->view.pos,

 ...
 */
void credits_joystick_func(double x, double y) { joystick_y = y; }
...
/*!
 ...
 \date    Modified: 2013-10-08
 */
void credits_joystick_button_func(int button) { go_back(); }
...
static void draw_credits_text(scalar_t time_step) {
  ...
  y_offset +=
      time_step * winsys_scale(CREDITS_SPEED +
                               time_step * joystick_y * CREDITS_JOYSTICK_SPEED);
  if (y_offset < 0) {
    y_offset = 0;
  }
...
void credits_register() {
  ...
  status |=
      add_keymap_entry(CREDITS, DEFAULT_CALLBACK, NULL, NULL, credits_key_cb);

...
static const double air_log_re[] = {-1, 0, 1, 2, 3, 4, 5, 6};
static const double air_log_drag_coeff[] = {2.25,  1.35,  0.6,   0.0,
                                            -0.35, -0.45, -0.33, -0.9};
...
scalar_t get_min_tux_speed() { return MIN_TUX_SPEED; }
...
void get_indices_for_point(scalar_t x, scalar_t z, int *x0, int *y0, int *x1,
                           int *y1) {
  scalar_t courseWidth, courseLength;
  scalar_t xidx, yidx;
  int nx, ny;
...
void find_barycentric_coords(scalar_t x, scalar_t z, index2d_t *idx0,
                             index2d_t *idx1, index2d_t *idx2, scalar_t *u,
                             scalar_t *v) {
  scalar_t xidx, yidx;
  int nx, ny;
  int x0, x1, y0, y1;
...
vector_t find_course_normal(scalar_t x, scalar_t z) {
  ...
  interp_nml = add_vectors(scale_vector(interp_factor, tri_nml),
...
static int compare_func(void const *a, void const *b) {
  ...
  scalar_t diff =
      squared_dist_to_starting_pointA - squared_dist_to_starting_pointB;
...
int first_tree_farther_than(scalar_t squared_dist) {
  ...
  do {
    if (squared_distance_to_starting_point(trees[middle].ray.pt) <
        squared_dist) {
      bottom = middle;
...
int first_item_farther_than(scalar_t squared_dist) {
  ...
  int middle = bottom + (top - bottom) / 2;
  do {
    if (squared_distance_to_starting_point(items[middle].ray.pt) <
        squared_dist) {
      bottom = middle;
      middle = bottom + (top - bottom) / 2;
      continue;
...
range_t potential_trees_in_collision(point_t pos) {
  /*in fact, we want to check the trees which are between two distances from the
   starting point. The difference between those two distances is (once squared)
   squared_diff_dist, which value was determined empirically with the necessity
   that at the end of the race, all the trees have to have been checked at least
   once
   squaredDistTux, which is the distance of tuc from its starting point is in
   the middle of those two distances         */
  ...
  int second_tree_index =
      first_tree_farther_than(squaredDistTux + squared_dist);
...
range_t potential_items_in_collision(point_t pos) {
  ...
  int second_item_index =
      first_item_farther_than(squaredDistTux + squared_dist);
...
/* This function order the trees in order ASC of their distance from tux
 starting point it is called just once at the begining of the race Then, instead
 of checking_collision with every tree of the map, with try to detect collision
 only with the trees which are ate the same distance  than tux from the starting
 point This should be a very important speedup */
...
bool_t check_tree_collisions(player_data_t *plyr, point_t pos,
  ...
  if (hit && MAG_SQD(subtract_points(last_collision_pos, pos)) >
                 COLLISION_COOLDOWN_DIST) {
    last_collision = True;
    speed = (int)sqrt(plyr->vel.x * plyr->vel.x + plyr->vel.y * plyr->vel.y +
                      plyr->vel.z * plyr->vel.z);
 ...
 */
static void adjust_for_tree_collision(player_data_t *plyr, point_t pos,
                                      vector_t *vel) {
  vector_t treeNml;
  point_t treeLoc;
  bool_t treeHit;
 ...
 * speed
 */
scalar_t adjust_velocity(vector_t *vel, point_t pos, plane_t surf_plane,
...
void adjust_position(point_t *pos, plane_t surf_plane,
  ...
  if (dist_from_surface < -MAX_SURFACE_PENETRATION) {
    *pos = move_point(*pos,
                      scale_vector(-MAX_SURFACE_PENETRATION - dist_from_surface,
                                   surf_plane.nml));
  }
  return;
}

static vector_t adjust_tux_zvec_for_roll(player_data_t *plyr, vector_t vel,
                                         vector_t zvec) {
  matrixgl_t rot_mat;
  ...
  if (plyr->control.is_braking) {
    make_rotation_about_vector_matrix(
        rot_mat, vel, plyr->control.turn_fact * BRAKING_ROLL_ANGLE);
  } else {
    make_rotation_about_vector_matrix(rot_mat, vel,
                                      plyr->control.turn_fact * MAX_ROLL_ANGLE);
  }

  return transform_vector(rot_mat, zvec);
}

static vector_t adjust_surf_nml_for_roll(player_data_t *plyr, vector_t vel,
                                         scalar_t fric_coeff, vector_t nml) {
  matrixgl_t rot_mat;
  scalar_t angle;
  scalar_t speed;
  scalar_t roll_angle;
  ...
  angle = plyr->control.turn_fact * roll_angle *
...
void adjust_orientation(player_data_t *plyr, scalar_t dtime, point_t pos,
                        vector_t vel, scalar_t dist_from_surface,
                        vector_t surf_nml) {
  ...
  time_constant = dist_from_surface > 0 ? TUX_ORIENTATION_AIRBORNE_TIME_CONSTANT
                                        : TUX_ORIENTATION_TIME_CONSTANT;

  plyr->orientation = interpolate_quaternions(plyr->orientation, new_orient,
  ...
  if (plyr->control.barrel_roll_factor) {
    make_rotation_about_vector_matrix(
        rot_mat, new_y, jump_from_time(plyr->control.barrel_roll_factor) * 360);
  ...
  if (plyr->control.flip_factor) {
    make_rotation_about_vector_matrix(
        rot_mat, new_x, jump_from_time(plyr->control.flip_factor) * 360);
    multiply_matrices(cob_mat, rot_mat, cob_mat);
  }
...
void generate_particles(player_data_t *plyr, scalar_t dtime, point_t pos,
                        scalar_t speed) {
  ...
  if (surf_weights[Snow] > 0.5 && pos.y < surf_y) {
    ...
    right_part_pt =
        move_point(right_part_pt, scale_vector(TUX_WIDTH / 2.0, xvec));

    left_part_pt =
        move_point(left_part_pt, scale_vector(-TUX_WIDTH / 2.0, xvec));

    right_part_pt.y = left_part_pt.y = surf_y;

    brake_particles = dtime * BRAKE_PARTICLES *
                      (plyr->control.is_braking ? 1.0 : 0.0) *
                      min(speed / PARTICLE_SPEED_FACTOR, 1.0);
    turn_particles =
        dtime * MAX_TURN_PARTICLES * min(speed / PARTICLE_SPEED_FACTOR, 1.0);
    roll_particles =
        dtime * MAX_ROLL_PARTICLES * min(speed / PARTICLE_SPEED_FACTOR, 1.0);

    left_particles =
        turn_particles * fabs(min(plyr->control.turn_fact, 0.)) +
        brake_particles +
        roll_particles * fabs(min(plyr->control.turn_animation, 0.));

    right_particles =
        turn_particles * fabs(max(plyr->control.turn_fact, 0.)) +
        brake_particles +
    ...
    left_part_vel =
        scale_vector(min(MAX_PARTICLE_SPEED, speed * PARTICLE_SPEED_MULTIPLIER),
                     left_part_vel);
...
vector_t calc_wind_force(vector_t player_vel) {
  ...
  drag_coeff = pow(
      10.0, lin_interp(air_log_re, air_log_drag_coeff, log10(re), table_size));
...
static vector_t calc_spring_force(scalar_t compression, vector_t vel,
  ...
  /* Stage 1 */
  spring_f_mag = min(compression, TUX_GLUTE_STAGE_1_COMPRESSIBILITY) *
                 TUX_GLUTE_STAGE_1_SPRING_COEFF;

  /* Stage 2 */
  spring_f_mag += max(0, min(compression - TUX_GLUTE_STAGE_1_COMPRESSIBILITY,
                             TUX_GLUTE_STAGE_2_COMPRESSIBILITY)) *
                  TUX_GLUTE_STAGE_2_SPRING_COEFF;

  /* Stage 3 */
  spring_f_mag += max(0., compression - TUX_GLUTE_STAGE_2_COMPRESSIBILITY -
                              TUX_GLUTE_STAGE_1_COMPRESSIBILITY) *
                  TUX_GLUTE_STAGE_3_SPRING_COEFF;

  /* Damping */
  spring_f_mag -=
      spring_vel * (compression <= TUX_GLUTE_STAGE_1_COMPRESSIBILITY
                        ? TUX_GLUTE_STAGE_1_SPRING_COEFF
...
static vector_t calc_net_force(player_data_t *plyr, point_t pos, vector_t vel) {
  ...
  surf_nml =
      adjust_surf_nml_for_roll(plyr, vel, surf_fric_coeff, orig_surf_nml);
  ...
  if (dist_from_surface <= -comp_depth) {
    ...
    nml_f =
        calc_spring_force(glute_compression, vel, surf_nml, &unclamped_nml_f);
  }
    ...
    if (plyr->airborne) {
      ...
      paddling_f = scale_vector(
          -1 * min(MAX_PADDLING_FORCE,
                   MAX_PADDLING_FORCE * (MAX_PADDLING_SPEED - speed) /
                       MAX_PADDLING_SPEED *
                       min(1.0, surf_fric_coeff / IDEAL_PADDLING_FRIC_COEFF)),
          fric_dir);
    }
  } else {
  ...
  net_force = add_vectors(
      ...
      add_vectors(
          ...
          add_vectors(
              ...
              add_vectors(
                  fric_f,
                  add_vectors(air_f, add_vectors(brake_f, paddling_f))))));
  ...
  while (!done) {
    ...
    for (;;) {
      ...
      if (solver.estimate_error != NULL) {
        ...
        print_debug(DEBUG_ODE, "pos_err: %g, vel_err: %g", tot_pos_err,
                    tot_vel_err);
...
void update_player_pos(player_data_t *plyr, scalar_t dtime) {
  ...
  adjust_orientation(plyr, dtime, plyr->pos, plyr->vel, dist_from_surface,
                     surf_nml);
  ...
  local_force =
      rotate_vector(quaternion_conjugate(plyr->orientation), plyr->net_force);

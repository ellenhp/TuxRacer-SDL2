...
// public:
    ...
    quadsquare(quadcornerdata* pcd, void * serialized_rep, size_t size);
    ~quadsquare();

    void rootinit(void);

    void *	GetSerializedRepresentation(size_t * out_size);
    void	AddHeightMap(const quadcornerdata& cd, const HeightMapInfo& hm);
	void	AddHeightMap(const quadcornerdata& cd, const HeightMapInfo& hm, bool serialized);

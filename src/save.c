    ...
    for (p=0;p<g_game.num_players;p++) {
        ...
        do
        {
                                        ...
                                        &score ) )
            {
                ...
                sprintf(request+length,"piste[%d]=%s&score[%d]=%d&herring[%d]=%d&time[%d]=%02d:%02d:%02d&order[%d]=%s",i,data->name,i,score,i,herring,i,minutes,seconds,hundredths,i,order);
                i++;
            }
        }

...
static void prefs_init(void)
{
	...
	if (is_on_ouya())
	{
		...
		gui_add_widget(prefs_back_button = create_button(get_back_text(), prefs_back_cb), &button_coord);
	}
	else
	{
	...
	if (is_on_ouya())
	{
		...
		button_coord.x_just = button_coord.y_just = CENTER_JUST;
		button_coord.x_coord_type = button_coord.y_coord_type = NORMALIZED_COORD;
		gui_add_widget(prefs_select_button = create_button(get_select_text(), prefs_select_cb), &button_coord);
	}

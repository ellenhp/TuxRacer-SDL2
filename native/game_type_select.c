...
void basic_tutorial_click_cb(int button, int mouse_x, int mouse_y, widget_bounding_box_t bb, input_type_t type, widget_t *widget)
{
...
void advanced_tutorial_click_cb(int button, int mouse_x, int mouse_y, widget_bounding_box_t bb, input_type_t type, widget_t *widget)
{
    g_game.current_event = NULL;
    g_game.current_cup = NULL;
    g_game.current_race = -1;
...
void practice_click_cb(int button, int mouse_x, int mouse_y, widget_bounding_box_t bb, input_type_t type, widget_t *widget)
{
#ifdef TARGET_OS_IPHONE
...
void credits_click_cb(int button, int mouse_x, int mouse_y, widget_bounding_box_t bb, input_type_t type, widget_t *widget)
{
...
void pref_click_cb(int button, int mouse_x, int mouse_y, widget_bounding_box_t bb, input_type_t type, widget_t *widget)
{
    set_game_mode(PREFS);
...
static void game_type_select_init(void)
{
    ...
    if (is_on_ouya())
    {
        ...
        gui_add_widget(main_quit_button = create_button(get_quit_text(), main_quit_cb), &button_coord);
    }
    else
    {
    ...
    if (is_on_ouya())
    {
        ...
        button_coord.x_just = button_coord.y_just = CENTER_JUST;
        button_coord.x_coord_type = button_coord.y_coord_type = NORMALIZED_COORD;
        gui_add_widget(main_select_button = create_button(get_select_text(), main_select_cb), &button_coord);
    }
